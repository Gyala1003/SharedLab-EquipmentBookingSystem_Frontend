  ...
  template: `
    <section class="mx-auto max-w-3xl space-y-6">
      ...
      <article class="card-surface overflow-hidden">
        <div
          class="bg-gradient-to-r from-[#111a3a] via-indigo-950 to-violet-900 px-6 py-8 text-white sm:px-9"
        >
          <div
        ...
        <div class="p-6 sm:p-9">
          <div
            class="mb-6 flex items-start gap-3 rounded-2xl border border-indigo-100 bg-indigo-50/60 p-4"
          >
            <span
            ...
            <div class="text-xs text-indigo-950">
              ...
              <p class="mt-0.5 text-indigo-700/80">
                Liên kết tạo mật khẩu mới sẽ được gửi trực tiếp về địa chỉ email tài khoản của bạn:
                <strong class="font-bold text-indigo-950">{{ email() }}</strong>
              </p>
            </div>
          </div>
          ...
          <form class="space-y-5" (submit)="sendResetEmail($event)">
            <div>
              ...
              <input
                class="input-shell bg-slate-50 font-semibold text-slate-700"
                type="email"
                [value]="email()"
                readonly
              />
            ...
            @if (sentSuccess()) {
              <div
                class="flex items-center gap-3 rounded-2xl border border-emerald-200 bg-emerald-50 p-4 text-xs font-semibold text-emerald-800"
              >
                <span
                ...
                <div>
                  <p class="text-sm font-bold">Đã gửi email thành công!</p>
                  <p class="mt-0.5">
                    Vui lòng kiểm tra hộp thư (bao gồm cả thư rác/spam) của {{ email() }} để tạo mật
                    khẩu mới.
                  </p>
                </div>
              </div>
      ...
      <p class="text-center text-xs leading-5 text-slate-400">
        Nếu bạn không còn truy cập được email này hoặc gặp sự cố, vui lòng liên hệ Quản trị viên hệ
        thống để được hỗ trợ cấp lại mật khẩu thủ công.
      </p>
    </section>
  `,
})
    ...
    this.authService.forgotPassword(userEmail, resetLink).subscribe({
      next: () => {
        ...
        this.toast.success(
          ...
          `Vui lòng kiểm tra hộp thư (${userEmail}) để nhận liên kết tạo mật khẩu mới.`,
        )
      },
      error: (err: any) => {
        this.loading.set(false)
