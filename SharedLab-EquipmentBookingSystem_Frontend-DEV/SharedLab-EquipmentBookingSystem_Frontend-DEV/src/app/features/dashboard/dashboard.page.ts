      ...
      <header class="flex flex-col justify-between gap-5 xl:flex-row xl:items-end">
        ...
        >
          ...
          <a
            ...
            class="flex h-10 items-center justify-center gap-2 rounded-xl bg-slate-900 px-4 text-xs font-bold text-white shadow-md transition hover:bg-slate-800"
          >
          ...
          <div class="mt-5 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            @for (rule of activePolicyRules().slice(0, 4); track $index) {
              <div class="rounded-2xl border border-slate-100 bg-slate-50/80 p-4">
                ...
                <p class="mt-1 text-xs leading-5 text-slate-600">{{ rule.desc }}</p>
              </div>
            }
          </div>
        </article>
