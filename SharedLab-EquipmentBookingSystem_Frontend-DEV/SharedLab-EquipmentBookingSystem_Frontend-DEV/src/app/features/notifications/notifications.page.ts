              ...
              <div class="flex flex-col gap-3 sm:flex-row">
                <div class="relative min-w-[220px]">
                  ...
                  <input
                    [(ngModel)]="searchText"
                    type="search"
                    placeholder="{{ 'notifications.searchPlaceholder' | t }}"
            ...
            @if (tab() === 'all') {
              ...
              >
                <p class="text-xs text-slate-400">
                  {{ 'common.page' | t: { current: pageNumber(), total: pageSize } }}
                </p>
                <div class="flex gap-2">
                  <button
                    ...
                    (click)="previousPage()"
                  >
                    <app-icon name="arrow-left" [size]="15" />{{ 'common.prev' | t }}
                  </button>
                  <button
                    type="button"
                    class="flex h-9 items-center gap-1 rounded-xl border border-slate-200 px-3 text-xs font-semibold text-slate-600 hover:bg-slate-50 disabled:opacity-40"
                    ...
                    (click)="nextPage()"
                  >
                    {{ 'common.next' | t }}<app-icon name="arrow-right" [size]="15" />
                  </button>
                </div>
              </div>
            }
          ...
          <article class="card-surface p-5">
            ...
            <div class="mt-4 space-y-2">
              ...
              @if (typeSummaries().length === 0) {
                <p class="py-6 text-center text-xs text-slate-400">{{ 'common.noData' | t }}</p>
              }
            </div>
          </article>
        </aside>
    ...
    @if (selected(); as notification) {
      ...
      >
        ...
        <div class="flex-1 overflow-y-auto p-6">
          ...
          @if (
            notification.notificationType === 'BookingReminder' &&
            notification.title.includes('Nhắc check-in booking #')
          ) {
            <div
              class="mt-6 rounded-2xl border border-indigo-100 bg-indigo-50 p-4 text-sm leading-6 text-indigo-700"
            >
              <strong>Check-in ngay bây giờ.</strong><br />
              Booking của bạn đã sắp đến giờ. Vui lòng check-in để xác nhận.
              <div class="mt-3">
                <a
                  [routerLink]="['/app/bookings', extractBookingId(notification.title)]"
          ...
          @if (
            notification.notificationType === 'BookingReminder' &&
            notification.title.includes('Nhắc check-out booking #')
          ) {
            <div
              class="mt-6 rounded-2xl border border-amber-200 bg-amber-50 p-4 text-sm leading-6 text-amber-900"
            >
              <strong>Check-out sắp đến giờ.</strong><br />
              Booking của bạn sắp kết thúc. Vui lòng check-out đúng giờ để tránh bị tính vi phạm trả
              muộn.
              <div class="mt-3">
                <a
                  [routerLink]="['/app/bookings', extractBookingId(notification.title)]"
                  class="btn-primary inline-flex bg-amber-600 hover:bg-amber-700"
                  (click)="selected.set(null)"
                >
