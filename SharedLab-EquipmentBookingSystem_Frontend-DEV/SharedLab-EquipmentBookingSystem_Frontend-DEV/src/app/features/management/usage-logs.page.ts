...
@Component({
  ...
  template: `<section class="space-y-6">
    ...
    <app-modal
      ...
      ><input class="input-shell" type="datetime-local" [(ngModel)]="checkoutTime" />
  ...
  protected readonly filtered = computed(() => {
    ...
    return this.items()
      .filter(
        (x) =>
          (!needle ||
            String(x.logId).includes(needle) ||
            String(x.bookingItemId).includes(needle)) &&
          (!this.usageStatus ||
            (this.usageStatus === 'active' ? !x.actualCheckout : !!x.actualCheckout)) &&
          (!this.reviewStatus || x.incidentReviewStatus === this.reviewStatus) &&
          (!this.from || toDateInput(new Date(x.actualCheckin)) >= this.from),
      )
