  ...
  template: `
    <section class="space-y-6">
        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 class="text-2xl font-black tracking-tight text-slate-950">
              {{ 'manageViolations.title' | t }}
            </h1>
            <p class="mt-1 text-xs font-medium text-slate-500">
              {{ 'manageViolations.subtitle' | t }}
            </p>
          </div>
          <div>
      ...
      <app-modal
        ...
        subtitle="Chọn booking từ danh sách hoặc nhập thủ công thông tin bên dưới."
      ...
      >
        <form class="grid gap-5" (ngSubmit)="create()">
          ...
          <div>
            ...
            @if (loadingForm()) {
              ...
              <p class="mt-1.5 text-xs text-slate-400">
                Chọn booking để tự động điền Booking ID và User ID bên dưới.
              </p>
            } @else {
              <p
                class="rounded-xl border border-amber-200 bg-amber-50 px-4 py-3 text-sm text-amber-800"
              >
                Không tải được danh sách booking. Vui lòng nhập thủ công Booking ID và User ID bên
                dưới.
              </p>
            }
          </div>
          ...
          <div class="flex justify-end gap-2 border-t border-slate-100 pt-4">
            ...
            <button class="btn-primary" [disabled]="saving() || !form.userId || !form.bookingId">
              @if (saving()) {
    ...
    this.api
      ...
      .subscribe({
        ...
        error: (err) => {
          this.saving.set(false)
          const detail =
            err?.error?.message || err?.message || 'Vui lòng kiểm tra lại Booking ID và User ID.'
