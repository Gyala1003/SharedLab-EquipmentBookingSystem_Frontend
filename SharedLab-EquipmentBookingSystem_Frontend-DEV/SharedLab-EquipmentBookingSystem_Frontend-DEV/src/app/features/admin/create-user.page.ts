    ...
    <section class="space-y-6">
      ...
      <div class="grid gap-6 xl:grid-cols-[1.3fr_.7fr]">
        <form class="card-surface overflow-hidden" (ngSubmit)="submit()">
          <div
            class="border-b border-slate-100 bg-gradient-to-r from-violet-50/80 to-cyan-50/60 px-6 py-5"
          >
          ...
          <div class="grid gap-5 p-6 md:grid-cols-2">
            ...
            <div>
              ...
              ><select class="input-shell" required [(ngModel)]="role" name="role">
            ...
            <div>
              ...
              ><select class="input-shell" required [(ngModel)]="departmentId" name="departmentId">
                ...
                <div class="mt-2.5 grid gap-2 sm:grid-cols-2">
                  @for (rule of passwordRules(); track rule.key) {
                    <div
                      ...
                      [ngClass]="rule.ok ? 'text-emerald-700' : 'text-slate-500'"
                    >
                      <span
...
export class CreateUserPage implements OnInit {
  ...
  protected readonly passwordMessage = computed(
    () =>
      [
  ...
  protected readonly checklist = computed(() => [
    {
      ...
      ok: this.fullName().trim().length >= 2 && this.username().trim().length >= 3,
    },
    { label: 'Email đúng định dạng', ok: /^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(this.email().trim()) },
    {
      label: 'Đã chọn vai trò và đơn vị',
      ok: this.role() !== null && this.departmentId() !== null,
    },
  ...
  ngOnInit(): void {
    this.api
      ...
      .subscribe({
        ...
        error: () => this.toast.error('Không tải được khoa/phòng ban'),
      })
    this.api
      .roles()
      .subscribe({
        next: (items) => this.roles.set(items),
        error: () => this.toast.error('Không tải được danh sách vai trò'),
      })
  }
  ...
  protected submit(): void {
    if (!this.isValid() || this.departmentId() === null || this.role() === null) {
      ...
      .subscribe({
        ...
        error: (err: any) => {
          ...
          const msg =
            ...
            err?.message ||
            'Username hoặc email có thể đã được sử dụng.'
