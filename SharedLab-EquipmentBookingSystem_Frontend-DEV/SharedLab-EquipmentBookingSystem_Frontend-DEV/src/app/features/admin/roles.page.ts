      ...
      @if (loading()) {
        <div class="grid gap-5 lg:grid-cols-3">
          @for (item of [1, 2, 3]; track item) {
            <div class="skeleton h-80 rounded-[30px]"></div>
          }
        </div>
        ...
        <div class="grid gap-5 lg:grid-cols-3">
          @for (role of roles(); track role.roleId; let index = $index) {
            <article
              class="relative overflow-hidden rounded-[30px] border border-white bg-white p-6 shadow-[0_22px_70px_rgba(15,23,42,.08)]"
            >
              <div
                class="absolute -top-16 -right-16 h-44 w-44 rounded-full blur-3xl"
                [class.bg-violet-200]="index === 0"
                [class.bg-cyan-200]="index === 1"
                [class.bg-emerald-200]="index === 2"
              ></div>
              <div class="relative">
                <div class="flex items-start justify-between">
                  <span
                    class="flex h-14 w-14 items-center justify-center rounded-2xl"
                    [class.bg-violet-100]="role.roleName === 'Admin'"
                    [class.text-violet-700]="role.roleName === 'Admin'"
                    [class.bg-cyan-100]="role.roleName === 'LabManager'"
                    [class.text-cyan-700]="role.roleName === 'LabManager'"
                    [class.bg-emerald-100]="role.roleName === 'Requester'"
                    [class.text-emerald-700]="role.roleName === 'Requester'"
                    ><app-icon [name]="roleIcon(role.roleName)" [size]="25" /></span
                  ><span
                    class="rounded-full bg-slate-100 px-3 py-1.5 text-xs font-black text-slate-500"
                    >Role ID {{ role.roleId }}</span
                  >
                </div>
                <p class="mt-7 text-[10px] font-black tracking-[.2em] text-slate-400 uppercase">
                  {{ role.roleName }}
                </p>
                ...
                <p class="mt-3 min-h-16 text-sm leading-6 text-slate-500">
                  {{ role.description || fallbackDescription(role.roleName) }}
                </p>
                <div class="mt-6 rounded-2xl bg-slate-50 p-4">
                  <p class="text-xs font-black text-slate-700">Phạm vi chính</p>
                  <ul class="mt-3 space-y-2">
                    @for (permission of permissions(role.roleName); track permission) {
                      <li class="flex items-start gap-2 text-xs leading-5 text-slate-500">
                        <span class="mt-0.5 text-emerald-500"
                          ><app-icon name="check" [size]="14" /></span
                        >{{ permission }}
                      </li>
                    }
                  </ul>
          ...
          <table class="table-shell">
            <thead>
              <tr>
                ...
                <th class="text-center">LabManager</th>
                <th class="text-center">Admin</th>
              </tr>
            </thead>
            <tbody>
              @for (row of matrix; track row.name) {
                <tr>
                  <td class="font-black text-slate-800">{{ row.name }}</td>
                  <td class="text-center">{{ row.requester ? '✓' : '—' }}</td>
...
export class RolesPage implements OnInit {
  ...
  ngOnInit(): void {
    this.api.roles().subscribe({
      ...
      error: () => {
        this.loading.set(false)
        this.toast.error('Không tải được danh sách vai trò')
      },
    })
  }
  protected roleLabel(role: string): string {
    return labelOf('userRole', role)
  }
  protected roleIcon(role: string): string {
    return role === ROLE.Admin ? 'shield' : role === ROLE.LabManager ? 'wrench' : 'user'
  }
