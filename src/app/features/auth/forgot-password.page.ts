  ...
  template: `
    ...
    >
      ...
      >
        ...
        <div
          class="mt-9 flex h-16 w-16 items-center justify-center rounded-3xl bg-gradient-to-br from-indigo-100 to-violet-100 text-indigo-600 shadow-inner"
        >
          <app-icon name="mail" [size]="28" />
        </div>
        <h1 class="mt-6 text-3xl font-bold tracking-[-0.035em] text-slate-950">Quên mật khẩu?</h1>
        <p class="mt-3 text-sm leading-6 text-slate-500">
          Nhập email của bạn. Hệ thống sẽ gửi một liên kết đặt lại mật khẩu có hiệu lực trong 1 giờ.
        </p>
        ...
        @if (sent) {
          <div class="mt-8 rounded-3xl border border-emerald-100 bg-emerald-50 p-6">
            ...
            <p class="mt-2 text-sm leading-6 text-emerald-700">
              Nếu email tồn tại trong hệ thống, liên kết đặt lại mật khẩu đã được gửi tới
              <strong>{{ email }}</strong
              >.
            </p>
            <button
              type="button"
              class="mt-5 text-sm font-bold text-emerald-800 hover:underline"
              (click)="sent = false"
            >
...
export class ForgotPasswordPage {
  ...
  protected async submit(): Promise<void> {
    ...
    this.errorMessage = ''
    try {
      await firstValueFrom(this.auth.forgotPassword({ email: this.email.trim() }))
      this.sent = true
    } catch (error) {
      this.errorMessage =
