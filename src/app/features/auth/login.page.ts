...
@Component({
  ...
  imports: [FormsModule, RouterLink, IconComponent, LanguageSwitcherComponent, TranslatePipe],
  template: `
        ...
        <section class="relative hidden overflow-hidden bg-[#111a3a] p-10 text-white lg:flex lg:flex-col xl:p-14">
          ...
          <div class="relative flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <div>
                <p class="text-xs font-bold uppercase tracking-[0.25em] text-cyan-300">{{ 'app.name' | translate }}</p>
                <p class="mt-0.5 text-base font-bold">{{ 'app.tagline' | translate }}</p>
              </div>
            </div>
            <app-language-switcher variant="dark-pill" />
          </div>
          ...
          <div class="relative my-auto max-w-xl py-16">
            <span class="inline-flex items-center gap-2 rounded-full border border-white/15 bg-white/10 px-4 py-2 text-xs font-semibold text-white/80 backdrop-blur">
              ...
              {{ 'auth.smartSpace' | translate }}
            </span>
            <h1 class="mt-8 text-5xl font-bold leading-[1.08] tracking-[-0.04em] xl:text-6xl">
              {{ 'auth.sloganLine1' | translate }}<br />
              {{ 'auth.sloganLine2' | translate }}<br />
              <span class="bg-gradient-to-r from-violet-300 to-cyan-300 bg-clip-text text-transparent">{{ 'auth.sloganLine3' | translate }}</span>
            </h1>
            <p class="mt-7 max-w-lg text-base leading-7 text-white/60 xl:text-lg">
              {{ 'auth.sloganDesc' | translate }}
            </p>
            ...
            <div class="mt-10 grid max-w-lg grid-cols-3 gap-3">
              @for (stat of stats(); track stat.key) {
                <div class="rounded-2xl border border-white/10 bg-white/[0.07] p-4 backdrop-blur">
                  ...
                  <p class="mt-1 text-xs text-white/45">{{ stat.key | translate }}</p>
                </div>
              }
            </div>
          </div>
          ...
          <div class="w-full max-w-[460px]">
            <div class="mb-8 flex items-center justify-between lg:hidden">
              <div class="flex items-center gap-3">
                ...
                <div>
                  <p class="text-xs font-bold uppercase tracking-[0.2em] text-indigo-500">{{ 'app.name' | translate }}</p>
                  <p class="text-sm font-bold text-slate-900">{{ 'app.tagline' | translate }}</p>
                </div>
              </div>
              <app-language-switcher variant="pill" />
            </div>

            <p class="text-sm font-semibold text-indigo-600">{{ 'auth.welcomeBack' | translate }}</p>
            <h2 class="mt-2 text-3xl font-bold tracking-[-0.035em] text-slate-950 sm:text-4xl">{{ 'auth.loginAccount' | translate }}</h2>
            <p class="mt-3 text-sm leading-6 text-slate-500">{{ 'auth.loginDesc' | translate }}</p>

            <form class="mt-9 space-y-5" (ngSubmit)="submit()" #form="ngForm">
              <label class="block">
                <span class="mb-2 block text-sm font-semibold text-slate-700">{{ 'auth.email' | translate }}</span>
                <div class="relative">
              ...
              <label class="block">
                <div class="mb-2 flex items-center justify-between gap-3">
                  <span class="text-sm font-semibold text-slate-700">{{ 'auth.password' | translate }}</span>
                  <a routerLink="/forgot-password" class="text-xs font-semibold text-indigo-600 hover:text-indigo-800">{{ 'auth.forgotPassword' | translate }}</a>
                </div>
                <div class="relative">
                  <span class="pointer-events-none absolute inset-y-0 left-0 flex items-center pl-4 text-slate-400">
...
export class LoginPage {
  ...
  protected stats() {
    return [
      { value: '24/7', key: 'auth.stats.access' },
      { value: '1', key: 'auth.stats.centralized' },
      { value: 'Realtime', key: 'auth.stats.realtime' },
    ]
  }
