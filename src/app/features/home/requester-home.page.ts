  ...
  template: `
    <section class="space-y-6">
      ...
      @if (loading()) {
        ...
        @if (accountWarning()) {
          <div class="flex flex-col gap-4 rounded-3xl border border-amber-200 bg-gradient-to-r from-amber-50 to-orange-50 p-5 sm:flex-row sm:items-center">
        ...
        <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
          @for (card of kpiCards(); track card.label) {
            <article class="card-surface group relative overflow-hidden p-5 transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-slate-900/10">
              <div class="absolute -right-10 -top-10 h-28 w-28 rounded-full opacity-55 blur-2xl" [class.bg-indigo-200]="card.tone === 'indigo'" [class.bg-cyan-200]="card.tone === 'cyan'" [class.bg-amber-200]="card.tone === 'amber'" [class.bg-rose-200]="card.tone === 'rose'"></div>
              <div class="relative flex items-start justify-between gap-4">
                <div>
                  <p class="text-sm font-medium text-slate-500">{{ card.label }}</p>
                  <p class="mt-3 text-3xl font-bold tracking-[-0.04em] text-slate-950">{{ card.value }}</p>
                  <p class="mt-2 text-xs text-slate-400">{{ card.note }}</p>
                </div>
                <div class="flex h-11 w-11 items-center justify-center rounded-2xl" [class.bg-indigo-50]="card.tone === 'indigo'" [class.text-indigo-600]="card.tone === 'indigo'" [class.bg-cyan-50]="card.tone === 'cyan'" [class.text-cyan-600]="card.tone === 'cyan'" [class.bg-amber-50]="card.tone === 'amber'" [class.text-amber-600]="card.tone === 'amber'" [class.bg-rose-50]="card.tone === 'rose'" [class.text-rose-600]="card.tone === 'rose'">
        ...
        <div class="grid gap-6 xl:grid-cols-[1.35fr_.65fr]">
            ...
            @if (upcomingBookings().length === 0) {
              ...
              <div class="divide-y divide-slate-100">
                @for (booking of upcomingBookings().slice(0, 4); track booking.bookingId) {
                  <button type="button" class="flex w-full items-center gap-4 px-5 py-4 text-left transition hover:bg-slate-50 sm:px-6" (click)="bookingDetail(booking)">
                    <div class="flex w-14 shrink-0 flex-col items-center rounded-2xl bg-[#111a3a] py-2 text-white">
                      <span class="text-[10px] font-semibold uppercase text-cyan-300">{{ booking.startTime | date: 'MMM' }}</span>
                      <span class="text-xl font-bold leading-6">{{ booking.startTime | date: 'dd' }}</span>
                    </div>
                    <div class="min-w-0 flex-1">
                      ...
                      <p class="mt-1 flex items-center gap-1.5 text-xs text-slate-400">
                        ...
                        {{ booking.startTime | date: 'HH:mm' }} – {{ booking.endTime | date: 'HH:mm, dd/MM/yyyy' }}
                      </p>
                    </div>
          ...
          <article class="card-surface p-5 sm:p-6">
            ...
            <div class="mt-7 flex items-center gap-5">
              <div class="relative flex h-28 w-28 shrink-0 items-center justify-center rounded-full" [style.background]="healthRing()">
                <div class="flex h-[82px] w-[82px] flex-col items-center justify-center rounded-full bg-white shadow-inner">
                  <span class="text-2xl font-bold text-slate-950">{{ healthScore() }}</span>
                  <span class="text-[10px] font-semibold uppercase text-slate-400">/ 100</span>
                </div>
              </div>
              <div class="min-w-0">
                  ...
                  <a routerLink="/app/notifications" class="flex items-start gap-4 px-5 py-4 transition hover:bg-slate-50 sm:px-6">
                    ...
                    <div class="min-w-0 flex-1">
                      <div class="flex items-start gap-3">
                        <p class="min-w-0 flex-1 font-semibold text-slate-900">{{ notification.title }}</p>
                        @if (!notification.isRead) { <span class="mt-1.5 h-2 w-2 shrink-0 rounded-full bg-indigo-500"></span> }
                      </div>
                      <p class="mt-1 line-clamp-2 text-sm leading-5 text-slate-500">{{ notification.message }}</p>
                      <p class="mt-2 text-[11px] text-slate-400">{{ notification.createdAt | date: 'HH:mm, dd/MM/yyyy' }}</p>
                    </div>
                  </a>
                }
              </div>
...
export class RequesterHomePage implements OnInit {
  ...
  protected readonly upcomingBookings = computed(() =>
    this.bookings()
      .filter((item) => item.status === 'Approved' && new Date(item.startTime).getTime() > Date.now())
  ...
  protected readonly statusText = computed(() => {
    ...
    return ({ 1: 'Active', 2: 'Inactive', 3: 'Restricted', 4: 'Locked' } as Record<number, string>)[status ?? 1] ?? 'Active'
  })
  protected readonly healthScore = computed(() =>
    Math.max(0, 100 - this.violationSummary().penaltyPoints * 5 - this.violationSummary().activeViolationCount * 5),
  )
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      notifications: this.workspace.notifications(user.userId, 1, 10).pipe(catchError(() => of([]))),
      unread: this.workspace.unreadCount(user.userId).pipe(catchError(() => of({ userId: user.userId, unreadCount: 0 }))),
      violations: this.workspace.violationSummary(user.userId).pipe(catchError(() => of({
        userId: user.userId,
      ...
      error: () => {
        this.loading.set(false)
        this.toast.error('Không tải được trang chủ', 'Hãy kiểm tra backend đang chạy tại cổng 5253.')
      },
    })
  }
