  ...
  template: `
    <section class="space-y-6">
      ...
      @else {
        ...
        <div class="grid gap-6 xl:grid-cols-2">          <article class="card-surface overflow-hidden"><header class="flex items-center justify-between border-b border-slate-100 px-5 py-5"><div><h2 class="font-black text-slate-950">{{ 'equipment.next30Days' | t }}</h2><p class="mt-1 text-xs text-slate-400">{{ 'equipment.scheduleSubtitle' | t }}</p></div><a routerLink="/app/calendar" [queryParams]="{ equipmentId: item()!.equipmentId }" class="text-xs font-black text-violet-600">{{ 'equipment.viewAll' | t }}</a></header>@if (events().length === 0) { <div class="p-5"><app-data-state [title]="'equipment.emptyScheduleTitle' | t" [message]="'equipment.emptyScheduleMsg' | t" icon="calendar" /></div> } @else { <div class="divide-y divide-slate-100">@for (event of events().slice(0,6); track event.eventType + event.sourceId) { <button class="flex w-full items-center gap-4 px-5 py-4 text-left hover:bg-slate-50" (click)="openEvent(event)"><div class="flex h-10 w-10 items-center justify-center rounded-2xl" [ngClass]="event.eventType === 'Maintenance' ? 'bg-amber-50 text-amber-600' : 'bg-indigo-50 text-indigo-600'"><app-icon [name]="event.eventType === 'Maintenance' ? 'wrench' : 'calendar'" [size]="18" /></div><div class="min-w-0 flex-1"><div class="flex flex-wrap items-center gap-2"><p class="truncate text-sm font-black text-slate-800">{{ event.title | t }}</p>@if (isFullRoomBooking(event)) { <span class="rounded-full bg-violet-100 px-2 py-0.5 text-[10px] font-bold text-violet-700">🏢 {{ 'equipment.fullRoomLock' | t }}</span> } @else if (isThisEquipmentBooking(event)) { <span class="rounded-full bg-cyan-100 px-2 py-0.5 text-[10px] font-bold text-cyan-700">🔬 {{ 'equipment.specificEquipLock' | t }}</span> }</div><p class="mt-1 text-xs text-slate-400">{{ event.startTime | date:'HH:mm dd/MM' }} – {{ event.endTime | date:'HH:mm dd/MM' }}</p></div><app-status-badge [value]="event.status" [domain]="event.eventType === 'Maintenance' ? 'maintenance' : 'booking'" /></button> }</div> }</article>
...
export class EquipmentDetailPage implements OnInit {
  ...
  protected isFullRoomBooking(event: CalendarEventResponse): boolean { return event.resources.some((r) => r.resourceType === 'LabRoom') }
  // API-004: BE CalendarResourceResponse không có equipmentId; dùng resourceType + resourceId.
  protected isThisEquipmentBooking(event: CalendarEventResponse): boolean { return event.resources.some((r) => r.resourceType === 'Equipment' && r.resourceId === this.id) }
