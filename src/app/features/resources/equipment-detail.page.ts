    ...
    <section class="space-y-6">
      ...
      @else {
        <app-page-header [title]="item()!.equipmentName" [subtitle]="'Thiết bị #' + item()!.equipmentId + ' · ' + (lab()?.labName || 'Phòng #' + item()!.labId)">
          ...
          @if (store.isManager()) { <a routerLink="/app/management/maintenances/new" [queryParams]="{ equipmentId: item()!.equipmentId }" class="btn-secondary"><app-icon name="wrench" [size]="17" /> Lên lịch bảo trì</a> }
