...
@Component({
  ...
  template: `
    <section class="space-y-6">
      ...
      @else if (!item()) { <app-data-state [title]="'equipments.notFound' | translate" message="Thiết bị có thể đã ngừng sử dụng hoặc không tồn tại." icon="microscope"><a routerLink="/app/equipments" class="btn-primary mt-5">{{ 'common.back' | translate }}</a></app-data-state> }
      @else {
        <app-page-header [title]="item()!.equipmentName" [subtitle]="'Thiết bị #' + item()!.equipmentId + ' · ' + (lab()?.labName || 'Phòng #' + item()!.labId)">
          <!-- Phân quyền: Manager và Admin không có nút đặt thiết bị -->
          @if (store.isRequester()) {
            <a routerLink="/app/bookings/new" [queryParams]="{ labId: item()!.labId, equipmentId: item()!.equipmentId }" class="btn-primary"><app-icon name="calendar-plus" [size]="17" /> {{ 'equipments.bookEquipment' | translate }}</a>
          }
          @if (store.isAdmin() || store.isManager()) { <a routerLink="/app/management/maintenances/new" [queryParams]="{ equipmentId: item()!.equipmentId }" class="btn-secondary"><app-icon name="wrench" [size]="17" /> {{ 'labDetail.scheduleMaintenance' | translate }}</a> }
          @if (store.isAdmin()) { <button class="btn-secondary" (click)="openEdit()"><app-icon name="edit" [size]="17" /> {{ 'common.save' | translate }}</button> }
        </app-page-header>
        ...
        <div class="grid gap-6 xl:grid-cols-[.8fr_1.2fr]">
          ...
          <article class="card-surface p-6 sm:p-7">
            <p class="text-xs font-black uppercase tracking-[.18em] text-violet-500">{{ 'equipmentDetail.techSpecs' | translate }}</p>
            <h2 class="mt-2 text-2xl font-black text-slate-950">{{ item()!.equipmentName }}</h2>
            <div class="mt-6 grid gap-4 sm:grid-cols-2">
              <div class="rounded-2xl bg-slate-50 p-4"><p class="text-[10px] font-black uppercase tracking-[.15em] text-slate-400">{{ 'equipmentDetail.labLocation' | translate }}</p><a [routerLink]="['/app/labs', item()!.labId]" class="mt-2 block font-black text-violet-700 hover:text-violet-900">{{ lab()?.labName || 'Phòng #' + item()!.labId }}</a></div>
              <div class="rounded-2xl bg-slate-50 p-4"><p class="text-[10px] font-black uppercase tracking-[.15em] text-slate-400">{{ 'equipmentDetail.serialNumber' | translate }}</p><p class="mt-2 font-black text-slate-800">EQ-{{ item()!.equipmentId.toString().padStart(4, '0') }}</p></div>
            </div>
            <div class="mt-5"><p class="text-xs font-black text-slate-700">{{ 'equipmentDetail.modelSpecs' | translate }}</p><p class="mt-2 whitespace-pre-line text-sm leading-7 text-slate-500">{{ item()!.modelSpecs || 'Chưa cập nhật thông số kỹ thuật.' }}</p></div>
            <div class="mt-5 rounded-2xl border border-cyan-100 bg-cyan-50/60 p-4"><p class="flex items-center gap-2 text-xs font-black text-cyan-800"><app-icon name="book-open" [size]="17" /> {{ 'equipmentDetail.guideline' | translate }}</p><p class="mt-2 whitespace-pre-line text-sm leading-6 text-cyan-900/65">{{ item()!.usageGuideline || 'Liên hệ LabManager để được hướng dẫn.' }}</p></div>
          </article>
        </div>

        <div class="grid gap-6 xl:grid-cols-2">
          <article class="card-surface overflow-hidden"><header class="flex items-center justify-between border-b border-slate-100 px-5 py-5"><div><h2 class="font-black text-slate-950">{{ 'equipmentDetail.schedule30Days' | translate }}</h2><p class="mt-1 text-xs text-slate-400">Booking và bảo trì của thiết bị</p></div><a routerLink="/app/calendar" [queryParams]="{ equipmentId: item()!.equipmentId }" class="text-xs font-black text-violet-600">Xem toàn bộ</a></header>@if (events().length === 0) { <div class="p-5"><app-data-state title="Lịch đang trống" message="Thiết bị chưa có sự kiện trong 30 ngày tới." icon="calendar" /></div> } @else { <div class="divide-y divide-slate-100">@for (event of events().slice(0,6); track event.eventType + event.sourceId) { <button class="flex w-full items-center gap-4 px-5 py-4 text-left hover:bg-slate-50" (click)="openEvent(event)"><div class="flex h-10 w-10 items-center justify-center rounded-2xl" [ngClass]="event.eventType === 'Maintenance' ? 'bg-amber-50 text-amber-600' : 'bg-indigo-50 text-indigo-600'"><app-icon [name]="event.eventType === 'Maintenance' ? 'wrench' : 'calendar'" [size]="18" /></div><div class="min-w-0 flex-1"><p class="truncate text-sm font-black text-slate-800">{{ event.title }}</p><p class="mt-1 text-xs text-slate-400">{{ event.startTime | date:'HH:mm dd/MM' }} – {{ event.endTime | date:'HH:mm dd/MM' }}</p></div><app-status-badge [value]="event.status" [domain]="event.eventType === 'Maintenance' ? 'maintenance' : 'booking'" /></button> }</div> }</article>
          <article class="card-surface overflow-hidden"><header class="border-b border-slate-100 px-5 py-5"><h2 class="font-black text-slate-950">{{ 'equipmentDetail.maintenanceHistory' | translate }}</h2><p class="mt-1 text-xs text-slate-400">Các lịch bảo trì gắn với thiết bị</p></header>@if (maintenances().length === 0) { <div class="p-5"><app-data-state title="Chưa có lịch bảo trì" message="Thiết bị chưa có bản ghi bảo trì." icon="wrench" /></div> } @else { <div class="divide-y divide-slate-100">@for (maintenance of maintenances().slice(0,6); track maintenance.maintenanceId) { <a [routerLink]="['/app/management/maintenances', maintenance.maintenanceId]" class="flex items-center gap-4 px-5 py-4 hover:bg-slate-50"><div class="flex h-10 w-10 items-center justify-center rounded-2xl bg-amber-50 text-amber-600"><app-icon name="wrench" [size]="18" /></div><div class="min-w-0 flex-1"><p class="font-black text-slate-800">Bảo trì #{{ maintenance.maintenanceId }}</p><p class="mt-1 text-xs text-slate-400">{{ maintenance.startTime | date:'dd/MM/yyyy HH:mm' }}</p></div><app-status-badge [value]="maintenance.status" domain="maintenance" /></a> }</div> }</article>
        </div>

        <app-modal [open]="editOpen()" title="Chỉnh sửa thiết bị" subtitle="Cập nhật thông tin kỹ thuật hoặc chuyển thiết bị sang phòng khác." (close)="editOpen.set(false)"><form class="grid gap-4" (ngSubmit)="save()"><div><label class="field-label">Tên thiết bị</label><input class="input-shell" required [(ngModel)]="form.equipmentName" name="equipmentName" /></div><div><label class="field-label">Phòng lab</label><select class="input-shell" required [(ngModel)]="form.labId" name="labId">@for (room of labs(); track room.labId) { <option [ngValue]="room.labId">{{ room.labName }}</option> }</select></div><div><label class="field-label">Model / thông số</label><textarea class="textarea-shell" [(ngModel)]="form.modelSpecs" name="modelSpecs"></textarea></div><div><label class="field-label">URL ảnh</label><input class="input-shell" [(ngModel)]="form.imageUrl" name="imageUrl" /></div><div><label class="field-label">Hướng dẫn</label><textarea class="textarea-shell" [(ngModel)]="form.usageGuideline" name="usageGuideline"></textarea></div><div class="flex justify-between gap-2"><button type="button" class="btn-secondary btn-danger" (click)="remove()"><app-icon name="trash" [size]="16" /> Ngừng sử dụng</button><div class="flex gap-2"><button type="button" class="btn-secondary" (click)="editOpen.set(false)">{{ 'common.cancel' | translate }}</button><button class="btn-primary" [disabled]="saving()">{{ saving() ? ('common.loading' | translate) : ('common.save' | translate) }}</button></div></div></form></app-modal>
      }
    </section>
  `,
})
