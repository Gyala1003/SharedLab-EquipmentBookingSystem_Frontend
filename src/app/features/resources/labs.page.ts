  ...
  template: `
    <section class="space-y-6">
      ...
      @if (loading()) {
        ...
        <div class="card-surface overflow-x-auto"><table class="table-shell"><thead><tr><th>{{ 'labs.labRoom' | t }}</th><th>{{ 'labs.location' | t }}</th><th>{{ 'labs.capacity' | t }}</th><th>{{ 'common.status' | t }}</th><th></th></tr></thead><tbody>@for (lab of labs(); track lab.labId) { <tr><td><p class="font-black text-slate-900">{{ lab.labName }}</p><p class="mt-1 text-xs text-slate-400">{{ lab.roomCode }}</p></td><td>{{ lab.location }}</td><td>{{ lab.capacity }} {{ 'common.people' | t }}</td><td><app-status-badge [value]="lab.status" domain="lab" /></td><td class="text-right"><a [routerLink]="['/app/labs', lab.labId]" class="font-black text-violet-600 hover:text-violet-800">{{ 'common.details' | t }} →</a></td></tr> }</tbody></table></div>
      }
...
export class LabsPage implements OnInit {
  ...
  protected load(): void { this.loading.set(true); this.api.searchLabs({ keyword: this.keyword || undefined, status: this.status || undefined, minimumCapacity: this.minimumCapacity ?? undefined, pageNumber: this.page(), pageSize: 12 }).subscribe({ next: (result) => { this.labs.set(result.items); this.totalPages.set(result.totalPages || 1); this.loading.set(false) }, error: () => { this.loading.set(false); this.toast.error('Không tải được danh sách phòng lab') } }) }
  ...
  protected create(): void { if (!this.form.managerId) { this.toast.info('Hãy chọn LabManager'); return } this.saving.set(true); this.api.createLab({ ...this.form, managerId: this.form.managerId, description: this.form.description || null, imageUrl: this.form.imageUrl || null, usageGuideline: this.form.usageGuideline || null }).subscribe({ next: () => { this.saving.set(false); this.createOpen.set(false); this.toast.success('Đã tạo phòng lab'); this.load() }, error: () => { this.saving.set(false); this.toast.error('Không thể tạo phòng lab') } }) }
