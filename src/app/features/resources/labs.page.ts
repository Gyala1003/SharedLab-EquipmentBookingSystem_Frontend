      ...
      <div class="filter-bar md:grid-cols-2 xl:grid-cols-[2fr_1fr_1fr_1fr_auto]">
        ...
        <div>
          ...
          ><select class="input-shell" [(ngModel)]="status">
            ...
            <option [value]="2">{{ 'labs.maintenance' | t }}</option>
            <option [value]="3">{{ 'labs.maintenance' | t }}</option>
            <option [value]="4">{{ 'common.all' | t }}</option>
          </select>
        </div>
        <div>
        ...
        <div>
          ...
          <div class="flex h-12 rounded-2xl bg-slate-100 p-1">
            <button
              ...
              [ngClass]="view() === 'grid' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-400'"
              (click)="view.set('grid')"
            >
              <app-icon name="grid" [size]="17" /></button
            ><button
              class="flex-1 rounded-xl text-xs font-black"
              [ngClass]="view() === 'table' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-400'"
      ...
      @if (loading()) {
        ...
        <div class="grid gap-5 md:grid-cols-2 xl:grid-cols-3">
          @for (lab of labs(); track lab.labId; let index = $index) {
            ...
            >
              <div
                class="relative h-44 overflow-hidden bg-gradient-to-br from-slate-900 via-indigo-950 to-violet-900"
              >
                <div
                ...
                ></div>
                <div
                  class="absolute inset-x-0 bottom-0 bg-gradient-to-t from-slate-950/85 to-transparent p-5 pt-14"
                >
