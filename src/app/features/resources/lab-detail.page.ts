    ...
    <section class="space-y-6">
      @if (loading()) {
            ...
            >
              @if (lab()!.imageUrl) {
                ...
                <div
                  ...
                  style="background-image: radial-gradient(circle at 20% 25%, #a78bfa, transparent 32%), radial-gradient(circle at 80% 80%, #22d3ee, transparent 28%)"
                ></div>
                <div class="absolute inset-0 flex items-center justify-center text-white/25">
                  <app-icon name="building" [size]="110" />
                </div>
              }
              <div
        ...
        <article class="card-surface p-6">
          ...
          <h2 class="mt-2 text-xl font-black text-slate-950">Quét mã để vào phòng</h2>
          @if (qrImageUrl()) {
            <div class="mt-4 flex flex-col items-center gap-4 sm:flex-row sm:items-start">
              <img
                [src]="qrImageUrl()"
                alt="QR điểm danh"
                class="h-[180px] w-[180px] shrink-0 rounded-2xl border border-slate-200 bg-white p-2"
              />
              <p class="text-sm leading-6 text-slate-500">
                In mã này dán tại cửa phòng lab, người dùng quét bằng camera điện thoại sẽ được đưa
                thẳng vào trang Chi tiết booking để điểm danh.
              </p>
            </div>
          } @else {
            <p class="mt-4 text-sm leading-6 text-slate-500">
              Bạn chưa có booking đang trong khung giờ hoạt động tại phòng này nên chưa thể tạo mã
              QR điểm danh.
            </p>
          }
        </article>
...
export class LabDetailPage implements OnInit {
  ...
  protected readonly qrLink = computed(() => {
    const bookingId = this.qrBookingId()
    return bookingId ? `${window.location.origin}/app/bookings/${bookingId}` : ''
  })
  protected readonly qrImageUrl = computed(() => {
    const link = this.qrLink()
    return link
  ...
  protected openEdit(): void {
    ...
    if (!this.managers().length)
      this.api
        .users({ roleName: 'LabManager', pageSize: 100 })
