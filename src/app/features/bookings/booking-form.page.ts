        ...
        <div class="space-y-6">
          ...
          @if (step() === 1) {
            <article class="card-surface p-5 sm:p-7"><div class="flex items-start gap-4"><div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-50 text-violet-600"><app-icon name="microscope" [size]="23" /></div><div><h2 class="text-xl font-black text-slate-950">{{ 'bookingForm.step1.title' | t }}</h2><p class="mt-1 text-sm text-slate-500">{{ 'bookingForm.step1.subtitle' | t }}</p></div></div>
              ...
              <div class="mt-7 flex justify-end"><button class="btn-primary" [disabled]="selected().length === 0" (click)="step.set(2)">{{ 'common.next' | t }} <app-icon name="arrow-right" [size]="17" /></button></div>
            </article>
          }

          @if (step() === 2) {
            <article class="card-surface p-5 sm:p-7"><div class="flex items-start gap-4"><div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-cyan-50 text-cyan-600"><app-icon name="clock" [size]="23" /></div><div><h2 class="text-xl font-black text-slate-950">{{ 'bookingForm.step2.title' | t }}</h2><p class="mt-1 text-sm text-slate-500">{{ 'bookingForm.step2.subtitle' | t }}</p></div></div><div class="mt-6 grid gap-4 sm:grid-cols-2"><div><label class="field-label">{{ 'bookingForm.step2.startLabel' | t }}</label><input class="input-shell" type="datetime-local" [(ngModel)]="startTime" /></div><div><label class="field-label">{{ 'bookingForm.step2.endLabel' | t }}</label><input class="input-shell" type="datetime-local" [(ngModel)]="endTime" /></div></div><div class="mt-5 flex flex-wrap gap-2"><button class="btn-secondary" (click)="checkAvailability()" [disabled]="checking()"><app-icon name="search" [size]="17" /> {{ checking() ? ('bookingForm.step2.checkingBtn' | t) : ('bookingForm.step2.checkBtn' | t) }}</button><a routerLink="/app/calendar" [queryParams]="{ labId: labId }" class="btn-secondary"><app-icon name="calendar" [size]="17" /> {{ 'bookingForm.step2.openCalendarBtn' | t }}</a></div>
              @if (availabilityMessage()) { <div class="mt-5 rounded-2xl border p-4" [ngClass]="available() ? 'border-emerald-200 bg-emerald-50 text-emerald-800' : 'border-amber-200 bg-amber-50 text-amber-800'"><div class="flex flex-wrap items-center gap-3"><p class="flex min-w-0 flex-1 items-center gap-2 font-black"><app-icon [name]="available() ? 'check' : 'alert'" [size]="18" /> {{ availabilityMessage() }}</p>@if(!available()){<button type="button" class="btn-secondary shrink-0" [disabled]="joiningWaitlist() || selected().length !== 1" (click)="joinWaitlist()"><app-icon name="hourglass" [size]="16" /> {{joiningWaitlist() ? ('bookingForm.step2.joiningWaitlistBtn' | t) : ('bookingForm.step2.joinWaitlistBtn' | t)}}</button>}</div>@if(!available()&&selected().length>1){<p class="mt-3 text-xs leading-5 opacity-75">{{ 'bookingForm.step2.waitlistRestriction' | t }}</p>}</div> }
              @if (suggestions().length) { <div class="mt-6"><p class="font-black text-slate-900">{{ 'bookingForm.step2.suggestionsTitle' | t }}</p><div class="mt-3 grid gap-3 md:grid-cols-2">@for (slot of suggestions(); track slot.startTime) { <button class="rounded-2xl border border-violet-200 bg-violet-50 p-4 text-left hover:bg-violet-100" (click)="chooseSlot(slot)"><p class="text-sm font-black text-violet-900">{{ slot.startTime | date:'HH:mm dd/MM/yyyy' }}</p><p class="mt-1 text-xs text-violet-700">{{ 'bookingForm.step2.to' | t }} {{ slot.endTime | date:'HH:mm dd/MM/yyyy' }}</p></button> }</div></div> }
              <div class="mt-7 flex justify-between"><button class="btn-secondary" (click)="step.set(1)"><app-icon name="arrow-left" [size]="17" /> {{ 'common.back' | t }}</button><button class="btn-primary" [disabled]="!validTime()" (click)="step.set(3)">{{ 'common.next' | t }} <app-icon name="arrow-right" [size]="17" /></button></div>
            </article>
          }
