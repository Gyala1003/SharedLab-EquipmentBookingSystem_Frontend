...
@Component({
  ...
  imports: [DatePipe, NgClass, FormsModule, RouterLink, PageHeaderComponent, IconComponent, DataStateComponent, TranslatePipe],
  template: `
    <section class="space-y-6">
      <app-page-header [title]="'bookingForm.title' | translate" [subtitle]="'bookingForm.subtitle' | translate">
      ...
      @if (store.isAdmin() || store.isManager()) {
        <div class="rounded-[24px] border border-rose-200 bg-rose-50 p-5 text-rose-900">
          <div class="flex gap-3">
            <app-icon name="alert" [size]="21" />
            <div>
              <p class="font-black">{{ 'bookingForm.restrictedNotice' | translate }}</p>
              <p class="mt-1 text-sm leading-6 text-rose-800/75">Tài khoản Quản trị viên (Admin) và Quản lý phòng lab (LabManager) không có quyền tạo yêu cầu đặt phòng/thiết bị.</p>
            </div>
          </div>
        </div>
      } @else {
        ...
        <div class="grid gap-6 xl:grid-cols-[1fr_320px]">
          <div class="space-y-6">
            <div class="card-surface p-3 sm:p-4"><div class="grid grid-cols-4 gap-2">@for (item of steps; track item.id) { <button type="button" class="relative rounded-2xl px-2 py-3 text-center transition" [ngClass]="step() === item.id ? 'bg-violet-600 text-white shadow-lg shadow-violet-200' : step() > item.id ? 'bg-emerald-50 text-emerald-700' : 'bg-slate-50 text-slate-400'" (click)="goTo(item.id)"><span class="mx-auto flex h-8 w-8 items-center justify-center rounded-full border border-current/20 text-xs font-black">{{ step() > item.id ? '✓' : item.id }}</span><span class="mt-2 hidden text-[10px] font-black uppercase tracking-[.08em] sm:block">{{ item.labelKey | translate }}</span></button> }</div></div>

            @if (step() === 1) {
              <article class="card-surface p-5 sm:p-7"><div class="flex items-start gap-4"><div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-violet-50 text-violet-600"><app-icon name="microscope" [size]="23" /></div><div><h2 class="text-xl font-black text-slate-950">{{ 'bookingForm.stepResource' | translate }}</h2><p class="mt-1 text-sm text-slate-500">Một booking chỉ được chứa tài nguyên thuộc cùng một phòng lab.</p></div></div>
                <div class="mt-6 grid gap-4 md:grid-cols-2"><button type="button" class="rounded-[24px] border p-5 text-left transition" [ngClass]="mode() === 'lab' ? 'border-violet-300 bg-violet-50 shadow-lg shadow-violet-100' : 'border-slate-200 bg-white hover:border-violet-200'" (click)="setMode('lab')"><div class="flex items-center gap-3"><span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-violet-600 shadow-sm"><app-icon name="building" [size]="22" /></span><div><p class="font-black text-slate-900">{{ 'bookingForm.modeLab' | translate }}</p><p class="mt-1 text-xs text-slate-500">Sử dụng toàn bộ không gian</p></div></div></button><button type="button" class="rounded-[24px] border p-5 text-left transition" [ngClass]="mode() === 'equipment' ? 'border-cyan-300 bg-cyan-50 shadow-lg shadow-cyan-100' : 'border-slate-200 bg-white hover:border-cyan-200'" (click)="setMode('equipment')"><div class="flex items-center gap-3"><span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-white text-cyan-600 shadow-sm"><app-icon name="microscope" [size]="22" /></span><div><p class="font-black text-slate-900">{{ 'bookingForm.modeEquipment' | translate }}</p><p class="mt-1 text-xs text-slate-500">Có thể chọn nhiều thiết bị</p></div></div></button></div>
                <div class="mt-6"><label class="field-label">{{ 'bookingForm.selectLab' | translate }} *</label><select class="input-shell" [(ngModel)]="labId" (ngModelChange)="onLabChange()"><option [ngValue]="null">Chọn phòng lab</option>@for (lab of labs(); track lab.labId) { <option [ngValue]="lab.labId" [disabled]="lab.status !== 'Available'">{{ lab.labName }} · {{ lab.roomCode }} · {{ lab.status }}</option> }</select></div>
                ...
                <div class="mt-7 flex justify-end"><button class="btn-primary" [disabled]="selected().length === 0" (click)="step.set(2)">{{ 'common.next' | translate }} <app-icon name="arrow-right" [size]="17" /></button></div>
              </article>
            }

            @if (step() === 2) {
              <article class="card-surface p-5 sm:p-7"><div class="flex items-start gap-4"><div class="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-cyan-50 text-cyan-600"><app-icon name="clock" [size]="23" /></div><div><h2 class="text-xl font-black text-slate-950">{{ 'bookingForm.stepTime' | translate }}</h2><p class="mt-1 text-sm text-slate-500">Thời gian sẽ được chuyển sang UTC trước khi gửi backend.</p></div></div><div class="mt-6 grid gap-4 sm:grid-cols-2"><div><label class="field-label">{{ 'bookingForm.startTime' | translate }} *</label><input class="input-shell" type="datetime-local" [(ngModel)]="startTime" /></div><div><label class="field-label">{{ 'bookingForm.endTime' | translate }} *</label><input class="input-shell" type="datetime-local" [(ngModel)]="endTime" /></div></div><div class="mt-5 flex flex-wrap gap-2"><button class="btn-secondary" (click)="checkAvailability()" [disabled]="checking()"><app-icon name="search" [size]="17" /> {{ checking() ? ('bookingForm.checking' | translate) : ('bookingForm.checkSlot' | translate) }}</button><a routerLink="/app/calendar" [queryParams]="{ labId: labId }" class="btn-secondary"><app-icon name="calendar" [size]="17" /> Mở lịch phòng</a></div>
...
export class BookingFormPage implements OnInit {
  ...
  protected readonly steps = [{ id: 1, labelKey: 'bookingForm.stepResource' }, { id: 2, labelKey: 'bookingForm.stepTime' }, { id: 3, labelKey: 'bookingForm.stepPurpose' }, { id: 4, labelKey: 'bookingForm.stepConfirm' }]
  ...
  ngOnInit(): void {
    if (this.store.isAdmin() || this.store.isManager()) {
      this.toast.info('Tài khoản Manager/Admin không sử dụng chức năng tạo booking.')
