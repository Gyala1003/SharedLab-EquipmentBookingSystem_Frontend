...
@Component({
  ...
  template: `
    <section class="space-y-6">
      <app-page-header
        title="{{ 'bookingForm.title' | t }}"
        subtitle="{{ 'bookingForm.subtitle' | t }}"
      >
        @if (!store.isRequester()) {
          <a routerLink="/app/calendar" class="btn-secondary"
            ><app-icon name="calendar" [size]="17" /> {{ 'bookingForm.checkCalendar' | t }}</a
          >
        }
      </app-page-header>
      ...
      @if (store.user()?.status !== 'Active') {
        <div class="rounded-[24px] border border-amber-200 bg-amber-50 p-5 text-amber-900">
          <div class="flex gap-3">
            ...
            <div>
              <p class="font-black">{{ 'bookingForm.accountStatusRestrictedTitle' | t }}</p>
              <p class="mt-1 text-sm leading-6 text-amber-800/75">
                {{
                  'bookingForm.accountStatusRestrictedMsg'
                    | t: { status: store.user()?.status ?? '' }
                }}
              </p>
            </div>
          </div>
        ...
        <div class="space-y-6">
          ...
          @if (step() === 1) {
            <article class="card-surface p-5 sm:p-7">
              ...
              <!-- Section label -->
              <div class="mt-7">
                <p class="mb-3 text-xs font-black tracking-widest text-slate-400 uppercase">
                  {{ 'bookingForm.step1.selectLabTitle' | t }}
                </p>

                <!-- Lab status legend -->
                <div
                  class="mb-4 flex flex-wrap items-center gap-3 text-[11px] font-bold text-slate-500"
                >
                  <span class="inline-flex items-center gap-1.5"
                    ><span class="h-2.5 w-2.5 rounded-full bg-emerald-400"></span>
                    {{ 'bookingForm.step1.statusAvailable' | t }}</span
                  >
                  <span class="inline-flex items-center gap-1.5"
                    ><span class="h-2.5 w-2.5 rounded-full bg-rose-400"></span>
                    {{ 'bookingForm.step1.statusInUse' | t }}</span
                  >
                  <span class="inline-flex items-center gap-1.5"
                    ><span class="h-2.5 w-2.5 rounded-full bg-amber-400"></span>
                    {{ 'bookingForm.step1.statusMaintenance' | t }}</span
                  >
                  <span class="inline-flex items-center gap-1.5"
                    ><span class="h-2.5 w-2.5 rounded-full bg-slate-300"></span>
                    {{ 'bookingForm.step1.statusUnavailable' | t }}</span
                  >
                </div>
                ...
                <div class="grid gap-3 sm:grid-cols-2">
                  @for (lab of labs(); track lab.labId) {
                    <button
                      ...
                      [ngClass]="{
                        'border-violet-400 bg-gradient-to-br from-violet-50 to-indigo-50 shadow-md ring-2 ring-violet-400/30':
                          labId() === lab.labId,
                        'border-slate-200 bg-white hover:border-violet-200 hover:bg-slate-50/80 hover:shadow-sm':
                          labId() !== lab.labId && lab.status === 'Available',
                    ...
                    >
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        <div class="mt-1.5">
                          @if (lab.status === 'Available') {
                            <span
                              class="inline-flex items-center gap-1 rounded-full border border-emerald-200 bg-emerald-50 px-2.5 py-0.5 text-[10px] font-black text-emerald-700"
                            >
                              <span
                            ...
                            <span
                              class="inline-flex items-center gap-1 rounded-full border border-rose-200 bg-rose-50 px-2.5 py-0.5 text-[10px] font-black text-rose-700"
                            >
                            ...
                            <span
                              class="inline-flex items-center gap-1 rounded-full border border-amber-200 bg-amber-50 px-2.5 py-0.5 text-[10px] font-black text-amber-700"
                            >
                            ...
                            <span
                              class="inline-flex items-center gap-1 rounded-full border border-slate-200 bg-slate-100 px-2.5 py-0.5 text-[10px] font-black text-slate-500"
                            >
                      ...
                      @if (labId() === lab.labId) {
                        <div
                          class="flex h-7 w-7 shrink-0 items-center justify-center rounded-full bg-violet-600 text-white shadow-sm"
                        >
              ...
              @if (labId() && selectedLab()) {
                <div
                  class="mt-6 space-y-5 rounded-[24px] border border-violet-200 bg-gradient-to-br from-violet-50/80 to-indigo-50/60 p-5"
                >
                  <!-- Lab info row -->
                  <div class="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p class="text-xs text-violet-600/70">
                        {{ selectedLab()!.roomCode }} · {{ 'labs.capacity' | t }}:
                        {{ selectedLab()!.capacity }} {{ 'common.people' | t }}
                      </p>
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <input
                      ...
                      [ngModel]="labNote"
                      (ngModelChange)="onLabNoteChange($event)"
                      placeholder="{{ 'bookingForm.step1.roomNotePlaceholder' | t }}"
                    />
                  </div>

                  <!-- Automatic Equipment list for selected Lab -->
                  <div
                    class="space-y-4 rounded-2xl border border-violet-200/80 bg-white/90 p-4 shadow-sm sm:p-5"
                  >
                    <div
                    ...
                    >
                      <div class="flex items-center gap-3">
                        <span
                          class="flex h-9 w-9 shrink-0 items-center justify-center rounded-xl bg-cyan-50 text-cyan-600 shadow-sm"
                        >
                          <app-icon name="microscope" [size]="18" />
                        </span>
                        <div>
                          <p class="text-sm font-black text-slate-900">
                            {{ 'bookingForm.step1.equipmentsInLabTitle' | t }}
                          </p>
                          <p class="text-xs text-slate-500">
                            {{
                              'bookingForm.step1.equipmentsInLabSubtitle'
                                | t: { name: (selectedLab()!.labName | t) }
                            }}
                          </p>
                        </div>
                      </div>
                      ...
                      @if (availableEquipments().length > 0) {
                        <div class="flex flex-wrap items-center gap-2">
                          <button
                            type="button"
                            class="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-bold text-slate-700 transition hover:bg-slate-100"
                          ...
                          <button
                            type="button"
                            class="inline-flex items-center gap-1 rounded-lg border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-bold text-rose-600 transition hover:border-rose-200 hover:bg-rose-50"
                            (click)="deselectAllEquipments()"
                          >
                            {{ 'common.deselectAll' | t }}
                          </button>
                          <span
                            class="rounded-full border border-cyan-200 bg-cyan-50 px-3 py-0.5 text-[11px] font-black text-cyan-700"
                          >
                            {{ selectedEquipmentCount() }}/{{ availableEquipments().length }}
                            {{ 'bookingForm.step1.equipmentsUnit' | t }}
                          </span>
                        </div>
                      }
                    </div>
                          ...
                          >
                            ...
                            <div class="min-w-0 flex-1">
                              <p class="truncate text-xs font-black text-slate-900">
                                {{ equipment.equipmentName | t }}
                              </p>
                              @if (equipment.status !== 'Available') {
                                <span
                                  class="mt-0.5 inline-block text-[10px] font-bold text-rose-500"
                                  >{{ 'bookingForm.step1.statusUnavailable' | t }}</span
                                >
                              } @else {
                                <span
                                  class="mt-0.5 inline-block text-[10px] font-bold text-emerald-600"
                                  >{{ 'bookingForm.step1.badgeAvailable' | t }}</span
                                >
                              }
                            </div>
                          </button>
                ...
                <div class="space-y-4 lg:col-span-6">
                      ...
                      <span class="text-xs font-bold text-violet-600">
                        {{
                          'bookingForm.step2.userQuota'
                            | t: { current: userDayBookingsCount(), max: 2 }
                        }}
                      </span>
                    </label>
                    <input
                  ...
                  @if (userMaxLimitReached()) {
                    <div class="rounded-2xl border border-amber-200 bg-amber-50 p-4 text-amber-900">
                      <div class="flex items-center gap-2.5 text-xs font-black sm:text-sm">
                  ...
                  <div class="rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                    <div class="flex items-center justify-between border-b border-slate-100 pb-3">
                      ...
                      <span
                        class="rounded-full bg-slate-100 px-2.5 py-0.5 text-[11px] font-black text-slate-600"
                      >
                        {{ dayEvents().length }} {{ 'bookingForm.step2.eventsCount' | t }}
                      </span>
                    </div>

                    <div class="mt-3 max-h-[280px] space-y-2.5 overflow-y-auto pr-1">
                      @if (dayEventsLoading()) {
                        ...
                        <div
                          class="rounded-xl border border-dashed border-slate-200 p-5 text-center"
                        >
                          <app-icon name="check" [size]="24" class="mx-auto text-emerald-500" />
                          <p class="mt-2 text-xs font-bold text-slate-700">
                            {{ 'bookingForm.step2.noEventsToday' | t }}
                          </p>
                          <p class="mt-1 text-[11px] text-slate-400">
                            {{ 'bookingForm.step2.allSlotsFree' | t }}
                          </p>
                        </div>
                      } @else {
                        @for (event of dayEvents(); track event.sourceId + event.eventType) {
                          <div
                            class="border-slate-150 flex items-center justify-between rounded-xl border bg-slate-50/80 p-3 transition hover:bg-slate-50"
                          >
                            <div class="min-w-0 flex-1">
                              <div class="flex items-center gap-2">
                                <span
                                  class="rounded-md bg-violet-100 px-2 py-0.5 text-[10px] font-black text-violet-800"
                                >
                            ...
                            <app-status-badge
                              [value]="event.status"
                              [domain]="
                                event.eventType === 'Maintenance' ? 'maintenance' : 'booking'
                              "
                            />
                          </div>
                        }
                  ...
                  @if (suggestedSlots().length > 0) {
                    <div
                      class="space-y-2.5 rounded-2xl border border-violet-200 bg-gradient-to-br from-violet-50/90 to-indigo-50/70 p-4 shadow-sm"
                    >
                      <div class="flex items-center justify-between">
                        <div class="flex items-center gap-2 text-xs font-black text-violet-950">
                          <app-icon name="sparkles" [size]="16" class="text-violet-600" />
                          <span>Gợi ý khung giờ rảnh từ hệ thống:</span>
                        </div>
                        <button
                          type="button"
                          class="text-[11px] font-bold text-violet-600 hover:text-violet-800"
                          (click)="suggestedSlots.set([])"
                        >
                          Đóng ✕
                        </button>
                      </div>
                      <div class="flex flex-wrap gap-2">
                        @for (slot of suggestedSlots(); track slot.startTime) {
                          <button
                            type="button"
                            class="inline-flex items-center gap-1.5 rounded-xl border border-violet-300 bg-white px-3 py-1.5 text-xs font-black text-violet-800 shadow-sm transition hover:border-violet-600 hover:bg-violet-600 hover:text-white"
                            (click)="applySuggestedSlot(slot)"
                          >
                            <app-icon name="clock" [size]="13" />
                            {{ slot.startTime | date: 'HH:mm' }} -
                            {{ slot.endTime | date: 'HH:mm' }} ({{
                              slot.startTime | date: 'dd/MM'
                            }})
                          </button>
                        }
                      </div>
                ...
                <div class="space-y-4 lg:col-span-6">
                    ...
                    @for (slot of slotsWithStatus(); track slot.id) {
                      ...
                      >
                        <div class="flex min-w-0 items-center gap-3.5">
                          <div
                            class="flex h-11 w-11 shrink-0 items-center justify-center rounded-xl text-xs font-black transition"
                            [ngClass]="{
                          ...
                          <div class="min-w-0">
                            <p class="text-sm leading-tight font-extrabold">
                              Slot {{ slot.slotNumber }} ({{ slot.startTimeStr }} –
                              {{ slot.endTimeStr }})
                            </p>
                            <p
                              class="mt-1 text-xs font-semibold"
                              [ngClass]="{
                                'font-bold text-slate-400': slot.isOccupied,
                            ...
                            >
                              @if (slot.isOccupied && slot.occupiedReason === 'maintenance') {
                                ...
                                slot.isOccupied && slot.occupiedReason === 'user_conflict'
                              ) {
                                {{
                                  'bookingForm.step2.slotUserConflict'
                                    | t: { id: slot.userBookingId || 0 }
                                }}
                              } @else if (
                                slot.isOccupied && slot.occupiedReason === 'limit_reached'
                              ) {
                        ...
                        @if (slot.isSelected && !slot.isOccupied) {
                          <div
                            class="flex h-7 w-7 items-center justify-center rounded-full bg-violet-600 text-white shadow-sm"
                          >
                  ...
                  @if (selectedSlotIds().length > 0) {
                    <div class="rounded-2xl border border-violet-200 bg-violet-50/80 p-4">
                      <div
                        class="flex items-center justify-between text-xs font-bold text-violet-900"
                      >
                        <span>{{ 'bookingForm.step2.selectedTimeRange' | t }}:</span>
                        <span
                          class="rounded-lg border border-violet-100 bg-white px-3 py-1 font-black text-violet-700 shadow-sm"
                        >
                          {{ formattedSelectedRange() }}
                        </span>
                      </div>
                      @if (selectedSlotRanges().length > 1) {
                        <p class="mt-2.5 text-[11px] leading-5 font-bold text-violet-700">
                          <app-icon
                            name="sparkles"
                            [size]="14"
                            class="mr-1 inline-block text-violet-600"
                          />
                          {{
                            'bookingForm.step2.nonContinuousNotice'
                              | t: { count: selectedSlotRanges().length }
                          }}
                        </p>
                      }
                    </div>
          ...
          @if (step() === 3) {
            <article class="card-surface p-5 sm:p-7">
              <div class="flex items-start gap-4">
                ...
                <div>
                  <h2 class="text-xl font-black text-slate-950">
                    {{ 'bookingForm.step3.title' | t }}
                  </h2>
                  <p class="mt-1 text-sm text-slate-500">{{ 'bookingForm.step3.subtitle' | t }}</p>
                </div>
              </div>
              <div class="mt-6 grid gap-3 sm:grid-cols-2">
                @for (purpose of purposesList(); track purpose.value) {
                  <button
                    type="button"
              ...
              <div class="mt-5">
                <label class="field-label">{{ 'bookingForm.step3.descLabel' | t }}</label
                ><textarea
                  class="textarea-shell min-h-36"
                  [(ngModel)]="purposeDescription"
                  placeholder="{{ 'bookingForm.step3.descPlaceholder' | t }}"
                ></textarea>
              </div>
              <div class="mt-7 flex justify-between">
                <button class="btn-secondary" (click)="step.set(2)">
                  <app-icon name="arrow-left" [size]="17" /> {{ 'common.back' | t }}</button
                ><button
                  class="btn-primary"
                  [disabled]="!purposeDescription.trim()"
                  (click)="step.set(4)"
                >
                  {{ 'bookingForm.step3.reviewBtn' | t }}
            ...
            <article class="card-surface p-5 sm:p-7">
                ...
                <div class="rounded-2xl bg-slate-50 p-5">
                  ...
                  @for (range of selectedSlotRanges(); track range.label) {
                    <div class="mt-2 border-b border-slate-200/60 pb-2 last:border-b-0 last:pb-0">
                      <p class="text-sm font-black text-slate-900">
                        {{ range.startTime | date: 'HH:mm' }} - {{ range.endTime | date: 'HH:mm' }}
                        <span class="text-xs font-bold text-violet-600">({{ bookingDate }})</span>
                      </p>
                      <p class="mt-0.5 text-xs font-semibold text-slate-500">{{ range.label }}</p>
                    </div>
                  }
                </div>
                <div class="rounded-2xl bg-slate-50 p-5">
                  <p class="text-[10px] font-black tracking-[.15em] text-slate-400 uppercase">
                    {{ 'bookingForm.step4.purposeLabel' | t }}
                  </p>
                  <p class="mt-2 font-black text-slate-900">{{ purposeLabel() }}</p>
                  <p class="mt-1 text-sm text-slate-500">
                    {{
                      'bookingForm.step4.priorityLevel' | t: { level: priorityFor(purposeKey()) }
                    }}
                  </p>
                </div>
              </div>
              <div class="mt-4 rounded-2xl border border-slate-200 p-5">
                <p class="text-xs font-black text-slate-700">
                  {{ 'bookingForm.step4.selectedResources' | t }}
                </p>
                    ...
                    <div class="flex items-center gap-3">
                      ...
                      <div class="min-w-0 flex-1">
                        ...
                        <p class="truncate text-xs text-slate-400">
                          {{ resource.note || ('bookingForm.step4.noNote' | t) }}
                        </p>
                      </div>
                    </div>
                  }
              ...
              <div class="mt-4 rounded-2xl bg-violet-50 p-5">
                <p class="text-xs font-black text-violet-800">
                  {{ 'bookingForm.step4.description' | t }}
                </p>
              ...
              <div class="mt-7 flex justify-between">
                ...
                >
                  ...
                  {{
                    submitting()
                      ? ('bookingForm.step4.submittingBtn' | t)
                      : ('bookingForm.step4.submitBtn' | t)
                  }}
                </button>
              </div>
            </article>
        ...
        <aside class="space-y-4 xl:sticky xl:top-28 xl:self-start">
          <article class="card-surface p-5">
            <p class="text-[10px] font-black tracking-[.16em] text-violet-500 uppercase">
              {{ 'bookingForm.sidebar.quickSummary' | t }}
            </p>
            <div class="mt-4 space-y-4">
              <div class="flex items-center justify-between text-sm">
                <span class="text-slate-500">{{ 'bookingForm.sidebar.labRoom' | t }}</span
                ><strong class="max-w-40 truncate text-slate-900">{{ selectedLabName() }}</strong>
              </div>
              <div class="h-px bg-slate-100"></div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-slate-500">{{ 'bookingForm.sidebar.resources' | t }}</span
                ><strong class="text-slate-900">{{ selected().length }}</strong>
              </div>
              <div class="h-px bg-slate-100"></div>
              <div class="flex items-center justify-between text-sm">
                <span class="text-slate-500">{{ 'bookingForm.sidebar.priority' | t }}</span
          ...
          >
            ...
            <p class="mt-4 font-black text-slate-900">{{ 'bookingForm.sidebar.tipTitle' | t }}</p>
            <p class="mt-2 text-sm leading-6 text-slate-600">
              {{ 'bookingForm.sidebar.tipBody' | t }}
            </p>
          </article>
        </aside>
      </div>
...
export class BookingFormPage implements OnInit {
  ...
  protected readonly slotsWithStatus = computed<SlotWithStatus[]>(() => {
    ...
    return FIXED_TIME_SLOTS.map((slot) => {
      ...
      let occupiedReason:
        'maintenance' | 'user_conflict' | 'booking' | 'limit_reached' | 'past_time' | undefined =
        undefined
      ...
      if (isPastTime) {
        ...
        isOccupied = true
        occupiedReason =
          matchingResourceEvent.eventType === 'Maintenance' ? 'maintenance' : 'booking'
      ...
      return {
        ...
        occupiedReason,
        maintenanceTitle:
          matchingResourceEvent?.eventType === 'Maintenance'
            ? matchingResourceEvent.title
            : undefined,
  ...
  protected readonly formattedSelectedRange = computed(() => {
    ...
    return (
      ranges.map((r) => `${r.startTimeStr} – ${r.endTimeStr}`).join(' & ') +
      ` (${this.bookingDate})`
    )
  })
  ...
  protected readonly selectedLab = computed(
    () => this.labs().find((lab) => lab.labId === this.labId()) ?? null,
  )
  protected readonly selectedLabName = computed(() => {
    this.languageStore.lang()
    const name = this.selectedLab()?.labName
    return name
      ? this.languageStore.t(name)
      : this.languageStore.t('bookingForm.sidebar.notSelected')
  })
  protected readonly selectedEquipmentCount = computed(
    () => this.selected().filter((r) => r.resourceType === 2).length,
  )
  ...
  ngOnInit(): void {
    ...
    forkJoin({
      ...
      rules: this.api.priorityRules(true).pipe(catchError(() => of([]))),
    }).subscribe({
      next: ({ labs, equipments, rules }) => {
        ...
        if (!preselectedId && targetEquipmentId) {
          const equip = equipments.find(
            (e) => e.equipmentId === targetEquipmentId && e.status !== 'Inactive',
          )
          if (equip) {
        ...
        if (preselectedId) {
          const lab = labs.find(
            (item) => item.labId === preselectedId && item.status !== 'Inactive',
          )
          if (lab) {
  ...
  protected isEquipmentSelected(id: number): boolean {
    return this.selected().some((item) => item.equipmentId === id)
  }
  protected isSelected(id: number): boolean {
    return this.isEquipmentSelected(id)
  }
  ...
  protected priorityFor(purpose: string): number {
    return (
      ...
      this.purposesList().findIndex((item) => item.key === purpose) + 1
    )
  }
  protected purposeKey(): string {
    return this.purposesList().find((item) => item.value === this.purposeType)?.key ?? 'Other'
  }
  protected purposeLabel(): string {
    return labelOf('purpose', this.purposeKey(), this.languageStore.lang())
  }
      ...
      forkJoin(createRequests$)
        ...
        .subscribe({
          ...
          const suggestions: SuggestedSlotResponse[] =
            ...
            (rawDetails?.['details'] as { suggestedSlots?: SuggestedSlotResponse[] } | undefined)
              ?.suggestedSlots ??
            []
