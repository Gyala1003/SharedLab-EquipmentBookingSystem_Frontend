...
@Component({
  ...
  template: `
    <section class="space-y-6">
      <app-page-header [title]="'myBookings.title' | translate" [subtitle]="'myBookings.subtitle' | translate">
        <!-- Phân quyền: Manager và Admin không có nút tạo booking -->
        @if (store.isRequester()) {
          <a routerLink="/app/bookings/new" class="btn-primary"><app-icon name="plus" [size]="17" /> {{ 'bookingForm.title' | translate }}</a>
        }
        <a routerLink="/app/calendar" class="btn-secondary"><app-icon name="calendar" [size]="17" /> {{ 'header.viewCalendar' | translate }}</a>
      </app-page-header>
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <div class="kpi-card"><p class="text-xs font-bold text-slate-400">Tổng booking</p><p class="mt-2 text-3xl font-black text-slate-950">{{ bookings().length }}</p></div>
        <div class="kpi-card"><p class="text-xs font-bold text-slate-400">Đang chờ duyệt</p><p class="mt-2 text-3xl font-black text-amber-600">{{ count('Pending') }}</p></div>
        <div class="kpi-card"><p class="text-xs font-bold text-slate-400">Đã duyệt</p><p class="mt-2 text-3xl font-black text-emerald-600">{{ count('Approved') }}</p></div>
        <div class="kpi-card"><p class="text-xs font-bold text-slate-400">Hoàn thành</p><p class="mt-2 text-3xl font-black text-indigo-600">{{ count('Completed') }}</p></div>
      </div>
      <div class="card-surface overflow-hidden">
        <div class="flex flex-col gap-4 border-b border-slate-100 p-5 sm:flex-row sm:items-center sm:justify-between">
          <div class="flex gap-2 overflow-x-auto">@for (tab of tabs; track tab.value) { <button class="shrink-0 rounded-xl px-3.5 py-2 text-xs font-black" [ngClass]="status === tab.value ? 'bg-violet-600 text-white' : 'bg-slate-50 text-slate-500 hover:bg-slate-100'" (click)="status = tab.value">{{ tab.label }} <span class="ml-1 opacity-70">{{ tab.value ? count(tab.value) : bookings().length }}</span></button> }</div>
          <div class="relative sm:w-72"><span class="absolute left-3.5 top-3 text-slate-400"><app-icon name="search" [size]="17" /></span><input class="input-shell h-11 pl-10" [(ngModel)]="keyword" [placeholder]="'myBookings.search' | translate" /></div>
        </div>
        @if (loading()) {
          <div class="p-6"><div class="skeleton h-72 rounded-2xl"></div></div>
        } @else if (filtered().length === 0) {
          <div class="p-6">
            <app-data-state [title]="'myBookings.empty' | translate" message="Chưa có booking nào phù hợp với bộ lọc hiện tại." icon="calendar">
              @if (store.isRequester()) {
                <a routerLink="/app/bookings/new" class="btn-primary mt-5">{{ 'bookingForm.title' | translate }}</a>
              }
            </app-data-state>
          </div>
        } @else {
          <div class="overflow-x-auto"><table class="table-shell"><thead><tr><th>Mã booking</th><th>Mục đích</th><th>Thời gian</th><th>Ưu tiên</th><th>Trạng thái</th><th>Ngày tạo</th><th></th></tr></thead><tbody>@for (booking of filtered(); track booking.bookingId) { <tr><td><span class="font-black text-slate-900">#BK-{{ booking.bookingId.toString().padStart(5,'0') }}</span></td><td><p class="font-bold text-slate-800">{{ labelOf('purpose', booking.purposeType) }}</p></td><td><p class="font-bold text-slate-700">{{ booking.startTime | date:'HH:mm dd/MM/yyyy' }}</p><p class="mt-1 text-xs text-slate-400">đến {{ booking.endTime | date:'HH:mm dd/MM/yyyy' }}</p></td><td><span class="rounded-full bg-violet-50 px-2.5 py-1 text-xs font-black text-violet-700">P{{ booking.priorityLevel ?? '—' }}</span></td><td><app-status-badge [value]="booking.status" domain="booking" /></td><td>{{ booking.createdAt | date:'dd/MM/yyyy' }}</td><td><a [routerLink]="['/app/bookings', booking.bookingId]" class="inline-flex h-9 w-9 items-center justify-center rounded-xl border border-slate-200 text-slate-500 hover:border-violet-200 hover:text-violet-700"><app-icon name="arrow-right" [size]="17" /></a></td></tr> }</tbody></table></div>
        }
      </div>
    </section>
  `,
