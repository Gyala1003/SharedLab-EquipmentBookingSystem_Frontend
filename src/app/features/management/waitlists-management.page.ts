              ...
              @for (item of filtered(); track item.waitlistId) {
                <tr>
                  ...
                  <td class="font-bold text-slate-800">
                    {{ item.userName || 'Chưa xác định người dùng' }}
                  </td>
                  <td>
...
export class WaitlistsManagementPage implements OnInit {
  ...
  protected filtered(): WaitlistResponse[] {
    return this.items()
      .filter((item) => {
        ...
        const matchesLab = !this.labId || item.labId === this.labId
        const matchesEquipment = !this.equipmentId || item.equipmentId === this.equipmentId

        let matchesDate = true
        if (this.requestedStart && this.requestedEnd && this.validRequestedRange()) {
          const filterStart = new Date(this.requestedStart).getTime()
          const filterEnd = new Date(this.requestedEnd).getTime()
          const itemStart = item.requestedStart ? new Date(item.requestedStart).getTime() : 0
          const itemEnd = item.requestedEnd ? new Date(item.requestedEnd).getTime() : 0
          matchesDate = itemStart >= filterStart && itemEnd <= filterEnd
        }

        return matchesStatus && matchesLab && matchesEquipment && matchesDate
      })
  ...
  protected loadQueue(): void {
    ...
    const startIso = new Date(this.requestedStart).toISOString()
    const endIso = new Date(this.requestedEnd).toISOString()
