        ...
        <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
          <div>
            <h1 class="text-2xl font-black tracking-tight text-slate-950">
              {{ canManage() ? ('maintenances.title' | t) : ('nav.maintenanceSchedule' | t) }}
            </h1>
          ...
          <div class="flex flex-wrap items-center gap-2">
            @if (canManage()) {
              <a
                ...
                ><app-icon name="plus" [size]="17" /> {{ 'maintenances.createSchedule' | t }}</a
              >
            }
        ...
        <header class="flex items-center justify-between border-b border-slate-100 px-5 py-5">
          ...
          <div class="inline-flex rounded-2xl bg-slate-100 p-1">
            <button
              ...
              [ngClass]="view() === 'table' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-500'"
            ...
            <button
              class="rounded-xl px-3 py-2 text-xs font-black"
              [ngClass]="view() === 'cards' ? 'bg-white text-cyan-700 shadow-sm' : 'text-slate-500'"
        ...
        @if (loading()) {
                ...
                @for (item of filtered(); track item.maintenanceId) {
                  <tr>
                    ...
                    <td>
                      ...
                      <p class="mt-1 text-xs text-slate-400">
                        {{ item.labId ? ('nav.items.labs' | t) : ('nav.items.equipments' | t) }}
                      </p>
                    </td>
                    <td>
                    ...
                    <td>
                      {{ labelOf('recurrence', item.recurrenceType, languageStore.lang()) }}
          ...
          <div class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-3">
            @for (item of filtered(); track item.maintenanceId) {
              <a
                [routerLink]="['/app/management/maintenances', item.maintenanceId]"
                class="rounded-[24px] border border-slate-200 p-5 transition hover:-translate-y-1 hover:border-cyan-200 hover:shadow-lg"
              >
                ...
                <div class="mt-4 rounded-2xl bg-slate-50 p-4">
                  ...
                  <p class="mt-1 text-xs text-slate-400">
                    {{ 'common.to' | t }} {{ item.endTime | date: 'HH:mm dd/MM/yyyy' }}
                  </p>
                </div>
              </a>
            }
...
export class MaintenancesPage implements OnInit {
        ...
        (item) =>
          (!this.labId() ||
            ...
            this.equipments().some(
              (eq) => eq.equipmentId === item.equipmentId && eq.labId === this.labId(),
            )) &&
  ...
  protected readonly cards = computed(() => [
    {
      status: '',
      label: this.languageStore.t('maintenances.totalSchedules'),
    ...
    {
      status: 'Scheduled',
      label: this.languageStore.t('maintenances.scheduled'),
      count: this.count('Scheduled'),
    ...
    {
      status: 'InProgress',
      label: this.languageStore.t('maintenances.inProgress'),
      count: this.count('InProgress'),
    ...
    {
      status: 'Completed',
      label: this.languageStore.t('maintenances.completed'),
      count: this.count('Completed'),
