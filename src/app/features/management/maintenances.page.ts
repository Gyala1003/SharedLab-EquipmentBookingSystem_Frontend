...
@Component({
  ...
  template: `<section class="space-y-6">
    <app-page-header
      [title]="canManage() ? ('maintenances.title' | t) : ('nav.maintenanceSchedule' | t)"
      [subtitle]="'maintenances.subtitle' | t"
    >
      @if (canManage()) {
    ...
    <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
      <div class="kpi-card">
        <p class="text-xs font-bold text-slate-400">{{ 'maintenances.totalSchedules' | t }}</p>
      ...
      <div class="kpi-card">
        <p class="text-xs font-bold text-slate-400">{{ 'maintenances.inProgress' | t }}</p>
    ...
    <div class="filter-bar md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_1fr_auto]">
      ...
      <div>
        ...
        ><select class="input-shell" [ngModel]="status()" (ngModelChange)="status.set($event)">
          ...
          <option value="Cancelled">{{ 'common.all' | t }}</option>
        </select>
      </div>
      <div>
      ...
      <header class="flex items-center justify-between border-b border-slate-100 px-5 py-5">
        <div>
          ...
          <p class="mt-1 text-xs text-slate-400">
            {{ filtered().length }} {{ 'dashboard.hours' | t }}
          </p>
        </div>
