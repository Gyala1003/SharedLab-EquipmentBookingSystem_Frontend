...
@Component({
  ...
  imports: [DatePipe, NgClass, FormsModule, PageHeaderComponent, IconComponent, ModalComponent, StatusBadgeComponent, DataStateComponent, TranslatePipe],
  template: `
    <section class="space-y-6">
      ...
      <div class="filter-bar md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto]">
        ...
        <div class="flex items-end">
          <button class="btn-primary w-full" (click)="load()"><app-icon name="filter" [size]="17" /> {{ 'common.apply' | t }}</button>
        </div>
      </div>
      <div class="grid gap-4 sm:grid-cols-3">
        <div class="kpi-card"><p class="text-xs font-bold text-slate-400">{{ 'incidents.pending' | t }}</p><p class="mt-2 text-3xl font-black text-amber-600">{{ count('Pending') }}</p></div>
        <div class="kpi-card"><p class="text-xs font-bold text-slate-400">{{ 'incidents.confirmed' | t }}</p><p class="mt-2 text-3xl font-black text-emerald-600">{{ count('Confirmed') }}</p></div>
        <div class="kpi-card"><p class="text-xs font-bold text-slate-400">{{ 'incidents.rejected' | t }}</p><p class="mt-2 text-3xl font-black text-rose-600">{{ count('Rejected') }}</p></div>
      </div>
      @if (loading()) {
        <div class="card-surface p-6"><div class="skeleton h-96 rounded-2xl"></div></div>
      } @else if (filtered().length === 0) {
        <app-data-state [title]="'incidents.noIncidentsTitle' | t" [message]="'incidents.noIncidentsSub' | t" icon="shield" />
      } @else {
        <div class="grid gap-5 lg:grid-cols-2">
          @for (item of filtered(); track item.logId) {
            <article class="card-surface p-5 sm:p-6">
              <div class="flex items-start justify-between gap-4">
                <div class="flex h-12 w-12 items-center justify-center rounded-2xl" [ngClass]="item.incidentReviewStatus === 'Pending' ? 'bg-amber-50 text-amber-600' : item.incidentReviewStatus === 'Confirmed' ? 'bg-emerald-50 text-emerald-600' : 'bg-rose-50 text-rose-600'">
                  <app-icon name="alert" [size]="22" />
                </div>
                <app-status-badge [value]="item.incidentReviewStatus" domain="incident" />
              </div>
              <p class="mt-5 text-lg font-black text-slate-950">{{ labelOf('incidentType', item.incidentStatus, languageStore.lang()) }}</p>
              <p class="mt-1 text-xs text-slate-400">UsageLog #{{ item.logId }} · BookingItem #{{ item.bookingItemId }}</p>
              ...
              <div class="mt-4 rounded-2xl bg-slate-50 p-4">
                ...
                <div class="mt-3 flex items-center justify-between text-xs">
                  ...
                  <strong class="text-slate-700">{{ item.actualCheckin | date: 'HH:mm dd/MM/yyyy' }}</strong>
                </div>
              </div>
              ...
              @if (item.incidentReviewStatus === 'Pending') {
                <div class="mt-5 flex gap-2">
                  <button class="btn-secondary btn-danger flex-1" (click)="openReview(item, 'reject')"><app-icon name="x" [size]="16" /> {{ 'incidents.reject' | t }}</button>
                  <button class="btn-primary flex-1" (click)="openReview(item, 'confirm')"><app-icon name="check" [size]="16" /> {{ 'incidents.confirm' | t }}</button>
                </div>
              }
            </article>
          }
...
export class IncidentsPage implements OnInit {
  ...
  protected readonly filtered = computed(() => this.items().filter((x) => !this.status || x.incidentReviewStatus === this.status))
  ...
  protected review(): void {
    ...
    this.api.reviewIncident(item.logId, this.reviewAction, this.reviewNote || null).subscribe({
      next: () => {
        ...
        this.toast.success(this.reviewAction === 'confirm' ? 'Đã xác nhận sự cố' : 'Đã từ chối sự cố')
        this.load()
      },
