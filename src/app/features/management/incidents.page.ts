...
@Component({
  ...
  template: `<section class="space-y-6">
    <app-page-header
      title="Duyệt sự cố sử dụng"
      subtitle="Xác nhận hoặc từ chối các sự cố được báo từ UsageLog. Xác nhận có thể phát sinh vi phạm và điểm phạt."
    />
    <div class="filter-bar md:grid-cols-2 xl:grid-cols-[1fr_1fr_1fr_auto]">
      ...
      <div class="flex items-end">
        <button class="btn-primary w-full" (click)="load()">
          <app-icon name="filter" [size]="17" /> Tải dữ liệu
        </button>
      </div>
    </div>
    <div class="grid gap-4 sm:grid-cols-3">
      <div class="kpi-card">
        <p class="text-xs font-bold text-slate-400">Chờ duyệt</p>
        <p class="mt-2 text-3xl font-black text-amber-600">{{ count('Pending') }}</p>
      </div>
      <div class="kpi-card">
        <p class="text-xs font-bold text-slate-400">Đã xác nhận</p>
        <p class="mt-2 text-3xl font-black text-emerald-600">{{ count('Confirmed') }}</p>
      </div>
      <div class="kpi-card">
        <p class="text-xs font-bold text-slate-400">Đã từ chối</p>
    ...
    @if (loading()) {
      ...
      <app-data-state
        ...
        message="Không có bản ghi sự cố phù hợp với khoảng thời gian và trạng thái đã chọn."
        icon="shield"
      />
    } @else {
        ...
        @for (item of filtered(); track item.logId) {
          <article class="card-surface p-5 sm:p-6">
            ...
            <p class="mt-4 min-h-12 text-sm leading-6 text-slate-600">
              {{ item.incidentDescription || 'Không có mô tả.' }}
            </p>
            ...
            @if (item.incidentReviewStatus === 'Pending') {
              <div class="mt-5 flex gap-2">
                ...
                ><button class="btn-primary flex-1" (click)="openReview(item, 'confirm')">
                  <app-icon name="check" [size]="16" /> Xác nhận
                </button>
              </div>
            }
          </article>
    ...
    <app-modal
      [open]="reviewOpen()"
      [title]="reviewAction === 'confirm' ? 'Xác nhận sự cố' : 'Từ chối sự cố'"
    ...
    >
      @if (reviewAction === 'confirm') {
        ...
        >
          <strong>Cảnh báo:</strong> sự cố được xác nhận có thể tạo vi phạm tự động và tăng điểm
          phạt người dùng.
        </div>
      }
      ...
      <div class="mt-5 flex justify-end gap-2">
        <button class="btn-secondary" (click)="reviewOpen.set(false)">Hủy</button
        ><button class="btn-primary" (click)="review()">Xác nhận thao tác</button>
      </div></app-modal
    >
  </section>`,
})
