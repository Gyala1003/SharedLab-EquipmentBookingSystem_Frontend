...
@Component({
  ...
  template: `<section class="space-y-6">
          ...
          <article class="card-surface p-5 transition hover:-translate-y-1 sm:p-6">
            <div class="flex flex-col gap-5 lg:flex-row lg:items-center">
              ...
              <div class="flex shrink-0 gap-2">
                ...
                >
                  @if (processingId() === item.bookingId) {
                    <span
    ...
    <app-modal
      ...
      [subtitle]="selectedApprove() ? 'Duyệt booking #' + selectedApprove()!.bookingId + '?' : ''"
      (close)="approveOpen.set(false)"
    >
      <p class="text-sm leading-6 text-slate-600">
        Bạn có chắc chắn muốn duyệt yêu cầu đặt lịch này không?
      </p>
      <div class="mt-5 flex justify-end gap-2">
        <button class="btn-secondary" (click)="approveOpen.set(false)">Hủy</button>
        <button class="btn-primary" [disabled]="processingId() !== null" (click)="confirmApprove()">
          Duyệt booking
        </button>
      </div>
    </app-modal>
...
export class PendingBookingsPage implements OnInit {
  ...
  protected readonly selectedApprove = signal<PendingBookingView | null>(null)
  ...
  protected openApprove(item: PendingBookingView): void {
    this.selectedApprove.set(item)
    this.approveOpen.set(true)
  }
  protected confirmApprove(): void {
    const item = this.selectedApprove()
    if (!item) return
    this.processingId.set(item.bookingId)
    this.api.approveBooking(item.bookingId).subscribe({
      ...
      error: (err: any) => {
        ...
        this.toast.error('Không thể duyệt booking', detail)
      },
    })
  }
  ...
  protected reject(): void {
    ...
    this.api.rejectBooking(item.bookingId, this.reason.trim()).subscribe({
      ...
      error: (err: any) => {
        this.processingId.set(null)
        const detail = err.error?.message || 'Không thể từ chối booking'
        this.toast.error('Không thể từ chối booking', detail)
      },
    })
  }
  private load(): void {
