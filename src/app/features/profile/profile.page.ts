      ...
      @if (store.user(); as user) {
        <div class="grid gap-6 xl:grid-cols-[360px_1fr]">
            ...
            <article class="card-surface p-6">
              <div class="flex items-center justify-between">
                <div class="flex items-center gap-2">
                  <app-icon name="user" [size]="20" /><span class="text-lg font-bold text-slate-950"
                    >My Profile</span
                  >
                </div>
                <span
                  class="rounded-full bg-gradient-to-r from-cyan-50 to-indigo-50 px-3 py-1.5 text-[10px] font-black tracking-[0.2em] text-indigo-600 uppercase"
                  >ShareLab</span
                >
              </div>
              ...
              <div class="mt-6 space-y-4">
                ...
                <div>
                  ...
                  ><input class="input-shell" type="text" [value]="user.email" readonly />
                </div>
                <div>
                ...
                <div>
                  ...
                  <div class="input-shell flex items-center gap-2 text-slate-400">
                    <app-icon name="calendar" [size]="16" /> {{ 'profile.notUpdated' | t }}
                  </div>
                </div>
                <div>
                  ...
                  <div class="input-shell flex items-center gap-2 text-slate-400">
                    <app-icon name="map-pin" [size]="16" /> {{ 'profile.notUpdated' | t }}
                  </div>
                </div>
                <div>
              ...
              <div class="mt-6 flex items-center justify-between border-t border-slate-100 pt-5">
                ...
                <div class="flex gap-2">
                  <a routerLink="/app/bookings/my" class="btn-secondary"
                    ><app-icon name="book-open" [size]="16" /> {{ 'profile.bookingHistory' | t }}</a
                  >
                  <button type="button" class="btn-secondary" (click)="openEdit(user)">
                    <app-icon name="edit" [size]="16" /> {{ 'profile.editProfile' | t }}
                  </button>
                </div>
              </div>
            </article>
          ...
          <div class="space-y-6">
                ...
                <div class="mt-6 rounded-2xl bg-slate-50 p-5">
                  @if (user.restrictionUntil) {
                    ...
                    <p class="mt-3 text-xs leading-5 text-slate-400">
                      {{ 'violations.restrictedUntil' | t }}
                    </p>
                  } @else {
                    <p class="text-lg font-bold text-slate-800">
                      {{ 'profile.noRestriction' | t }}
                    </p>
            ...
            <article
              class="flex flex-col gap-4 rounded-3xl border border-indigo-100 bg-gradient-to-r from-indigo-50 to-violet-50 p-5 sm:flex-row sm:items-center sm:p-6"
            >
              <div
        ...
        <app-modal
          ...
          subtitle="Một số trường quan trọng có thể cần Admin duyệt lại."
    ...
    this.api
      ...
      .subscribe({
        ...
        error: () => {
          ...
          this.editOpen.set(false)
          this.toast.error(
            'Không thể tự cập nhật hồ sơ',
            'Tài khoản của bạn có thể chưa được cấp quyền tự sửa thông tin, vui lòng liên hệ Admin.',
          )
        },
      })
  }
  ...
  protected profileItems(user: {
    ...
    return [
      ...
      },
      {
        ...
        value: user.departmentName || this.languageStore.t('profile.notUpdated'),
        icon: 'building',
      },
    ]
  }
