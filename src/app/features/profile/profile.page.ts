  ...
  template: `
    <section class="space-y-6">
      @if (store.user(); as user) {
        <header class="flex flex-col justify-between gap-4 rounded-[28px] border border-cyan-100/80 bg-white p-6 shadow-sm shadow-cyan-950/5 md:flex-row md:items-center">
          <div>
            <div class="inline-flex items-center gap-2 rounded-full border border-cyan-200 bg-cyan-50/80 px-3 py-1 text-xs font-semibold text-cyan-800">
          ...
          <aside class="space-y-6">
            <article class="card-surface p-6">
              <div class="flex flex-col items-center text-center">
                <div class="flex h-24 w-24 items-center justify-center rounded-[32px] bg-gradient-to-br from-cyan-600 via-teal-500 to-indigo-600 text-3xl font-black text-white shadow-xl shadow-cyan-500/20">
                ...
                <div class="mt-3 flex flex-wrap justify-center gap-2">
                  <span class="rounded-full bg-cyan-50 px-3 py-1 text-xs font-bold text-cyan-700">{{ roleLabel(user.roleName) }}</span>
          ...
          <div class="space-y-6">
            <article class="card-surface overflow-hidden">
              ...
              <div class="divide-y divide-slate-100">
                @for (item of profileItems(user); track item.label) {
                  <div class="flex flex-col gap-2 p-5 sm:flex-row sm:items-center sm:justify-between sm:px-6">
                    <div class="flex items-center gap-3">
                      <span class="flex h-10 w-10 items-center justify-center rounded-2xl bg-slate-100 text-slate-500"><app-icon [name]="item.icon" [size]="18" /></span>
                      <div>
                    ...
                    <span class="self-start rounded-xl border border-slate-200 bg-slate-50 px-3 py-1 text-xs font-semibold text-slate-600 sm:self-auto">{{ 'profile.verified' | t }}</span>
                  </div>
                }
              </div>
            </article>
            ...
            <article class="grid gap-6 md:grid-cols-2">
              <div class="card-surface p-6">
                ...
                <p class="mt-4 text-xs leading-5 text-slate-500">{{ statusDescription(statusText()) }}</p>
              </div>
              ...
              <div class="card-surface p-6">
                <div class="flex items-center justify-between">
                  <div>
                    ...
                    <p class="text-xs text-slate-400">{{ 'profile.penaltyPoints' | t }}: {{ user.penaltyPoints }}</p>
                  </div>
                  <span class="text-2xl font-black" [class.text-emerald-600]="user.penaltyPoints === 0" [class.text-amber-500]="user.penaltyPoints > 0 && user.penaltyPoints < 10" [class.text-rose-600]="user.penaltyPoints >= 10">
                    {{ legitPoint(user) }}/100
                  </span>
                </div>
                <div class="mt-4 h-2.5 overflow-hidden rounded-full bg-slate-100">
                  <div class="h-full rounded-full transition-all duration-500" [style.width.%]="legitPoint(user)" [class.bg-emerald-500]="user.penaltyPoints === 0" [class.bg-amber-500]="user.penaltyPoints > 0 && user.penaltyPoints < 10" [class.bg-rose-500]="user.penaltyPoints >= 10"></div>
                </div>
              </div>
            </article>
            ...
            <article class="card-surface p-6">
              <div class="flex items-center justify-between">
                ...
                <span class="rounded-full px-3 py-1 text-xs font-bold" [class.bg-emerald-50]="!user.restrictionUntil" [class.text-emerald-700]="!user.restrictionUntil" [class.bg-rose-50]="!!user.restrictionUntil" [class.text-rose-700]="!!user.restrictionUntil">
              ...
              <div class="mt-5 rounded-2xl border border-slate-100 bg-slate-50/70 p-4">
                @if (user.restrictionUntil) {
                  <p class="font-bold text-rose-700">{{ 'profile.restrictedUntil' | t }}: {{ user.restrictionUntil }}</p>
...
export class ProfilePage {
  ...
  protected requestPasswordReset(): void {
    ...
    this.authService.forgotPassword(user.email, resetLink).subscribe({
      next: () => {
        ...
        this.toast.success('Đã gửi email đặt lại mật khẩu', `Vui lòng kiểm tra hộp thư (${user.email}) để nhận liên kết tạo mật khẩu mới.`)
      },
      error: () => {
        this.sendingReset.set(false)
        this.toast.error('Không thể gửi yêu cầu đặt lại mật khẩu', 'Vui lòng thử lại sau hoặc liên hệ quản trị viên.')
      },
    })
  }
  ...
  protected statusDescription(status: string): string {
    ...
    } as Record<string, string>)[status] ?? 'Trạng thái tài khoản không xác định.'
  }
