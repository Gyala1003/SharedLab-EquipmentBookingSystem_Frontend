...
@Component({
  ...
  template: `
          ...
          @if (loading()) {
            <div class="divide-y divide-slate-100">
              @for (item of [1, 2, 3, 4, 5]; track item) {
                <div class="flex animate-pulse gap-4 px-5 py-5 sm:px-6"><div class="h-12 w-12 rounded-2xl bg-slate-100"></div><div class="flex-1"><div class="h-4 w-2/5 rounded bg-slate-100"></div><div class="mt-3 h-3 w-4/5 rounded bg-slate-100"></div><div class="mt-2 h-3 w-1/3 rounded bg-slate-100"></div></div></div>
              }
            </div>
            ...
            <div class="divide-y divide-slate-100">
              @for (notification of filteredNotifications(); track notification.notificationId) {
                ...
                >
                  <div class="mt-0.5 flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl transition group-hover:scale-105" [class.bg-emerald-50]="tone(notification.notificationType) === 'emerald'" [class.text-emerald-600]="tone(notification.notificationType) === 'emerald'" [class.bg-rose-50]="tone(notification.notificationType) === 'rose'" [class.text-rose-600]="tone(notification.notificationType) === 'rose'" [class.bg-amber-50]="tone(notification.notificationType) === 'amber'" [class.text-amber-600]="tone(notification.notificationType) === 'amber'" [class.bg-indigo-50]="tone(notification.notificationType) === 'indigo'" [class.text-indigo-600]="tone(notification.notificationType) === 'indigo'" [class.bg-cyan-50]="tone(notification.notificationType) === 'cyan'" [class.text-cyan-600]="tone(notification.notificationType) === 'cyan'">
                    ...
                    <div class="flex items-start gap-3">
                      <div class="min-w-0 flex-1">
                        <div class="flex flex-wrap items-center gap-2">
                          ...
                          <span class="rounded-full bg-white px-2.5 py-1 text-[9px] font-bold uppercase tracking-wider text-slate-500 shadow-sm">{{ typeLabel(notification.notificationType) }}</span>
                        </div>
                        <p class="mt-2 line-clamp-2 text-sm leading-6 text-slate-500">{{ notification.message }}</p>
                        <p class="mt-2 flex items-center gap-1.5 text-[11px] text-slate-400"><app-icon name="clock" [size]="13" />{{ notification.createdAt | date: 'HH:mm, dd/MM/yyyy' }}</p>
                      </div>
                      @if (!notification.isRead) { <span class="mt-2 h-2.5 w-2.5 shrink-0 rounded-full bg-indigo-500 shadow-[0_0_0_4px_rgba(99,102,241,.12)]"></span> }
                    </div>
                  </div>
                  <span class="mt-4 hidden text-slate-300 transition group-hover:translate-x-1 group-hover:text-indigo-500 sm:block"><app-icon name="arrow-right" [size]="18" /></span>
                </button>
              }
            </div>
            ...
            <div class="mt-4 space-y-2">
              @for (summary of typeSummaries(); track summary.type) {
                <button type="button" class="flex w-full items-center gap-3 rounded-2xl p-3 text-left transition hover:bg-slate-50" (click)="typeFilter = summary.type">
                  <span class="flex h-9 w-9 items-center justify-center rounded-xl" [class.bg-emerald-50]="tone(summary.type) === 'emerald'" [class.text-emerald-600]="tone(summary.type) === 'emerald'" [class.bg-rose-50]="tone(summary.type) === 'rose'" [class.text-rose-600]="tone(summary.type) === 'rose'" [class.bg-amber-50]="tone(summary.type) === 'amber'" [class.text-amber-600]="tone(summary.type) === 'amber'" [class.bg-indigo-50]="tone(summary.type) === 'indigo'" [class.text-indigo-600]="tone(summary.type) === 'indigo'" [class.bg-cyan-50]="tone(summary.type) === 'cyan'" [class.text-cyan-600]="tone(summary.type) === 'cyan'"><app-icon [name]="icon(summary.type)" [size]="17" /></span>
                  <span class="min-w-0 flex-1 truncate text-xs font-semibold text-slate-600">{{ typeLabel(summary.type) }}</span>
    ...
    @if (selected(); as notification) {
      ...
      <aside class="fixed inset-y-0 right-0 z-50 flex w-full max-w-[480px] flex-col bg-white shadow-2xl shadow-slate-950/25">
        <div class="flex items-center justify-between border-b border-slate-100 px-6 py-5">
          <div><p class="text-xs font-bold uppercase tracking-[0.15em] text-indigo-500">Chi tiết thông báo</p><p class="mt-1 text-sm text-slate-400">#{{ notification.notificationId }}</p></div>
          <button type="button" class="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 text-slate-500 hover:bg-slate-50" (click)="selected.set(null)"><app-icon name="x" [size]="19" /></button>
        </div>
        <div class="flex-1 overflow-y-auto p-6">
          <div class="flex h-16 w-16 items-center justify-center rounded-[22px]" [class.bg-emerald-50]="tone(notification.notificationType) === 'emerald'" [class.text-emerald-600]="tone(notification.notificationType) === 'emerald'" [class.bg-rose-50]="tone(notification.notificationType) === 'rose'" [class.text-rose-600]="tone(notification.notificationType) === 'rose'" [class.bg-amber-50]="tone(notification.notificationType) === 'amber'" [class.text-amber-600]="tone(notification.notificationType) === 'amber'" [class.bg-indigo-50]="tone(notification.notificationType) === 'indigo'" [class.text-indigo-600]="tone(notification.notificationType) === 'indigo'" [class.bg-cyan-50]="tone(notification.notificationType) === 'cyan'" [class.text-cyan-600]="tone(notification.notificationType) === 'cyan'"><app-icon [name]="icon(notification.notificationType)" [size]="29" /></div>
          <span class="mt-6 inline-flex rounded-full bg-slate-100 px-3 py-1.5 text-[10px] font-bold uppercase tracking-wider text-slate-500">{{ typeLabel(notification.notificationType) }}</span>
          <h2 class="mt-4 text-2xl font-bold leading-tight tracking-[-0.025em] text-slate-950">{{ notification.title }}</h2>
          <p class="mt-4 whitespace-pre-line text-sm leading-7 text-slate-600">{{ notification.message }}</p>
          <div class="mt-7 rounded-2xl bg-slate-50 p-4">
            <div class="flex items-center justify-between gap-4 text-sm"><span class="text-slate-400">Thời gian gửi</span><strong class="text-right text-slate-700">{{ notification.createdAt | date: 'HH:mm, dd/MM/yyyy' }}</strong></div>
          ...
          @if (notification.notificationType.toLowerCase().includes('violation')) {
            <div class="mt-6 rounded-2xl border border-rose-100 bg-rose-50 p-4 text-sm leading-6 text-rose-700"><strong>Thông báo liên quan vi phạm.</strong><br />Xem trạng thái và điểm phạt ở trang Tài khoản cá nhân.</div>
          }
        </div>
        <div class="border-t border-slate-100 p-5">
          <button type="button" class="flex h-12 w-full items-center justify-center gap-2 rounded-2xl bg-[#111a3a] text-sm font-bold text-white hover:bg-[#17234c]" (click)="selected.set(null)">Đóng chi tiết</button>
        </div>
      </aside>
    }
  `,
...
export class NotificationsPage implements OnInit {
  ...
  protected load(): void {
    ...
    const request = this.tab() === 'unread'
      ? this.workspace.unreadNotifications(user.userId)
      : this.workspace.notifications(user.userId, this.pageNumber(), this.pageSize)
    request.subscribe({
      next: (items) => {
        this.notifications.set([...items].sort((a, b) => +new Date(b.createdAt) - +new Date(a.createdAt)))
  ...
  protected markAllRead(): void {
    ...
    this.workspace.markAllNotificationsRead(user.userId).subscribe({
      ...
      error: (error: unknown) => {
        this.actionLoading.set(false)
        this.toast.error('Không thể cập nhật', error instanceof ApiError ? error.message : undefined)
      },
    })
  }
  ...
  protected nextPage(): void {
    if (this.notifications().length < this.pageSize) return
    this.pageNumber.update((page) => page + 1)
    this.load()
  }
  ...
  protected tone(type: string): 'emerald' | 'rose' | 'amber' | 'indigo' | 'cyan' {
    ...
    if (normalized.includes('approve') || normalized.includes('available') || normalized.includes('waitlist')) return 'emerald'
    ...
    this.workspace.markNotificationRead(notification.notificationId).subscribe({
      next: () => {
        this.notifications.update((items) =>
          items.map((item) => item.notificationId === notification.notificationId ? { ...item, isRead: true } : item),
        )
        this.selected.update((item) => item ? { ...item, isRead: true } : item)
        this.badge.decrement()
        if (this.tab() === 'unread') this.notifications.update((items) => items.filter((item) => item.notificationId !== notification.notificationId))
      },
