  ...
  template: `
    <section class="space-y-6">
      <app-page-header
        ...
        subtitle="Tìm kiếm, phân quyền và theo dõi trạng thái toàn bộ tài khoản trong hệ thống."
      >
        <a routerLink="/app/admin/users/new" class="btn-primary"
          ><app-icon name="user-plus" [size]="17" /> Tạo người dùng</a
        >
      </app-page-header>
      ...
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        <article class="kpi-card">
          <div class="flex items-center justify-between">
            <p class="text-xs font-bold text-slate-400">Tổng tài khoản</p>
          ...
          <p class="mt-2 text-xs text-slate-400">Theo bộ lọc hiện tại</p>
        </article>
        <article class="kpi-card">
          <div class="flex items-center justify-between">
            <p class="text-xs font-bold text-slate-400">Đang hiển thị</p>
          ...
          <p class="mt-2 text-xs text-slate-400">Trang {{ page() }} / {{ totalPages() || 1 }}</p>
        </article>
        <article class="kpi-card">
          <div class="flex items-center justify-between">
            <p class="text-xs font-bold text-slate-400">Tổng điểm phạt</p>
          ...
          <p class="mt-2 text-xs text-slate-400">Trong danh sách đang hiển thị</p>
        </article>
        <article class="kpi-card">
          <div class="flex items-center justify-between">
          ...
          <p class="mt-2 text-xs text-slate-400">Đơn vị có trong hệ thống</p>
        </article>
      </div>
      ...
      <div class="filter-bar md:grid-cols-2 xl:grid-cols-[2fr_1fr_1.2fr_1fr_auto]">
        ...
        <div>
          ...
          ><select class="input-shell" [(ngModel)]="status" (ngModelChange)="applyFilters()">
            ...
            <option [ngValue]="3">Bị hạn chế</option>
                  ...
                  <tr>
                    <td>
                      <div class="flex items-center gap-3">
                        <div
                          class="flex h-11 w-11 shrink-0 items-center justify-center rounded-2xl bg-linear-to-br from-violet-100 to-cyan-100 text-sm font-black text-violet-700"
                        >
                    ...
                    <td class="text-right">
                      <a
                        ...
                        >Chi tiết <app-icon name="arrow-right" [size]="15"
                      /></a>
                    </td>
                  </tr>
                }
      ...
      @if (totalPages() > 1) {
        ...
        >
          <p class="px-2 text-xs font-bold text-slate-400">
            Hiển thị {{ users().length }} / {{ totalCount() }} tài khoản
          </p>
