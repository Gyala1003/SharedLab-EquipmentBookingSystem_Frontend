...
@Component({
  ...
  template: `
    <section class="space-y-6">
      <app-page-header [title]="'admin.usersTitle' | translate" [subtitle]="'admin.usersSub' | translate">
        <a routerLink="/app/admin/users/new" class="btn-primary"><app-icon name="user-plus" [size]="17" /> {{ 'admin.createUser' | translate }}</a>
      </app-page-header>
      ...
      <div class="grid gap-4 sm:grid-cols-2 xl:grid-cols-4">
        ...
        <article class="kpi-card"><div class="flex items-center justify-between"><p class="text-xs font-bold text-slate-400">{{ 'violations.penaltyPoints' | translate }}</p><span class="rounded-xl bg-rose-50 p-2 text-rose-600"><app-icon name="alert" [size]="18" /></span></div><p class="mt-3 text-3xl font-black text-rose-600">{{ pagePenaltyPoints() }}</p><p class="mt-2 text-xs text-slate-400">Trong danh sách đang hiển thị</p></article>
      ...
      <div class="filter-bar md:grid-cols-2 xl:grid-cols-[2fr_1fr_1.2fr_1fr_auto]">
        <div><label class="field-label">{{ 'common.search' | translate }}</label><div class="relative"><span class="pointer-events-none absolute left-4 top-3.5 text-slate-400"><app-icon name="search" [size]="18" /></span><input class="input-shell pl-11" [(ngModel)]="keyword" (keyup.enter)="applyFilters()" [placeholder]="'users.search' | translate" /></div></div>
        <div><label class="field-label">{{ 'profile.role' | translate }}</label><select class="input-shell" [(ngModel)]="roleName"><option value="">{{ 'myBookings.all' | translate }}</option><option value="Admin">Admin</option><option value="LabManager">LabManager</option><option value="Requester">Requester</option></select></div>
        <div><label class="field-label">{{ 'profile.department' | translate }}</label><select class="input-shell" [(ngModel)]="departmentId"><option [ngValue]="null">{{ 'myBookings.all' | translate }}</option>@for (department of departments(); track department.departmentId) { <option [ngValue]="department.departmentId">{{ department.departmentName }}</option> }</select></div>
        <div><label class="field-label">{{ 'labs.status' | translate }}</label><select class="input-shell" [(ngModel)]="status"><option [ngValue]="null">{{ 'myBookings.all' | translate }}</option><option [ngValue]="1">Đang hoạt động</option><option [ngValue]="2">Ngừng hoạt động</option><option [ngValue]="3">Bị hạn chế</option><option [ngValue]="4">Bị khóa</option></select></div>
        <div class="flex items-end"><button type="button" class="btn-primary w-full" (click)="applyFilters()"><app-icon name="filter" [size]="17" /> {{ 'common.filter' | translate }}</button></div>
      </div>
      ...
      @if (loading()) {
        ...
        <app-data-state icon="users" [title]="'users.empty' | translate" message="Không có tài khoản nào phù hợp với bộ lọc hiện tại."><a routerLink="/app/admin/users/new" class="btn-primary mt-5">{{ 'admin.createUser' | translate }}</a></app-data-state>
      } @else {
        <article class="card-surface overflow-hidden">
          <div class="overflow-x-auto">
            <table class="table-shell">
              <thead><tr><th>{{ 'users.name' | translate }}</th><th>{{ 'profile.role' | translate }}</th><th>{{ 'profile.department' | translate }}</th><th>Điểm phạt</th><th>{{ 'labs.status' | translate }}</th><th>Hạn chế đến</th><th></th></tr></thead>
              <tbody>
                @for (user of users(); track user.userId) {
                  <tr>
                    ...
                    <td><span class="rounded-full border border-indigo-100 bg-indigo-50 px-3 py-1 text-xs font-black text-indigo-700">{{ 'roles.' + user.roleName | translate }}</span></td>
                    ...
                    <td class="text-right"><a [routerLink]="['/app/admin/users', user.userId]" class="inline-flex items-center gap-1 font-black text-violet-600 hover:text-violet-800">{{ 'equipments.detail' | translate }} <app-icon name="arrow-right" [size]="15" /></a></td>
                  </tr>
                }
              </tbody>
            </table>
      ...
      @if (totalPages() > 1) {
        <div class="flex flex-wrap items-center justify-between gap-3 rounded-2xl bg-white p-3 shadow-sm">
          ...
          <div class="flex items-center gap-2"><button class="btn-secondary" [disabled]="page() <= 1" (click)="goPage(page() - 1)"><app-icon name="chevron-left" [size]="16" /> {{ 'common.back' | translate }}</button><span class="rounded-xl bg-slate-100 px-4 py-2.5 text-xs font-black text-slate-600">{{ page() }} / {{ totalPages() }}</span><button class="btn-secondary" [disabled]="page() >= totalPages()" (click)="goPage(page() + 1)">{{ 'common.next' | translate }} <app-icon name="chevron-right" [size]="16" /></button></div>
        </div>
      }
    </section>
  `,
