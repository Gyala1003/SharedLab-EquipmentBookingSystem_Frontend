          ...
          @if (!editingId()) {
            <div>
              ...
              >
                ...
                @for (opt of availablePurposeOptions(); track opt.value) {
                  <option [ngValue]="opt.value">{{ opt.label }}</option>
                }
              </select>
              @if (form.purposeType === 4) {
                <input
                  class="input-shell mt-2"
                  type="text"
                  [(ngModel)]="customPurposeName"
                  name="customPurposeName"
                  placeholder="Nhập tên mục đích tùy chỉnh..."
                />
              }
            </div>
          }
...
export class PriorityRulesPage implements OnInit {
  ...
  protected customPurposeName = ''
  protected form = { purposeType: null as number | null, priorityLevel: 1, description: '' }

  protected readonly allPurposeOptions = [
    { value: 1, key: 'ResearchProject', label: 'Dự án nghiên cứu' },
    { value: 2, key: 'CoursePractice', label: 'Thực hành môn học' },
    { value: 3, key: 'SelfStudy', label: 'Tự học' },
    { value: 4, key: 'Other', label: 'Khác' },
  ]

  protected readonly availablePurposeOptions = computed(() => {
    const existingTypes = new Set<number>(
      this.rules().map((r) => {
        const p = String(r.purposeType).trim()
        if (p === 'ResearchProject' || p === '1') return 1
        if (p === 'CoursePractice' || p === '2') return 2
        if (p === 'SelfStudy' || p === '3') return 3
        if (p === 'Other' || p === '4') return 4
        return Number(r.purposeType) || 4
      }),
    )
    return this.allPurposeOptions.filter((opt) => !existingTypes.has(opt.value))
  })
  ...
  protected save(): void {
    ...
    if (!this.editingId() && this.form.purposeType === 4 && this.customPurposeName.trim()) {
      desc = desc ? `${this.customPurposeName.trim()} - ${desc}` : this.customPurposeName.trim()
    }
