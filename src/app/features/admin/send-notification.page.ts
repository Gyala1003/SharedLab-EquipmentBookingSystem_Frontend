  ...
  template: `
    <section class="space-y-6">
      <app-page-header
        ...
        subtitle="Soạn thông báo trực tiếp cho một người dùng. Backend hiện chưa hỗ trợ gửi hàng loạt theo vai trò hoặc khoa."
      >
      ...
      <div class="grid gap-6 xl:grid-cols-[1.1fr_.9fr]">
        <form class="card-surface overflow-hidden" (ngSubmit)="send()">
          <div
            class="border-b border-slate-100 bg-gradient-to-r from-violet-50/80 to-cyan-50/70 px-6 py-5"
          >
          ...
          @if (selectedUser(); as recipient) {
            <article class="card-surface p-6">
              ...
              <div class="mt-4 flex items-center gap-4">
                <div
                  class="flex h-14 w-14 items-center justify-center rounded-2xl bg-gradient-to-br from-violet-100 to-cyan-100 font-black text-violet-700"
                >
...
export class SendNotificationPage implements OnInit {
  ...
  ngOnInit(): void {
    this.api.users({ pageNumber: 1, pageSize: 100 }).subscribe({
      next: (response) => this.users.set(response.items),
      error: () => this.toast.error('Không tải được danh sách người nhận'),
    })
  }
