...
export class SystemService {
  ...
  usersMap(forceRefresh = false): Observable<Map<number, string>> {
    ...
    this.usersMapCache$ = this.loadAllUsers().pipe(
      map((items) => {
  ...
  private loadAllUsers(
    pageNumber = 1,
    accumulated: UserManagementResponse[] = [],
  ): Observable<UserManagementResponse[]> {
    return this.users({ pageNumber, pageSize: 100 }).pipe(
      switchMap((res) => {
        const allItems = [...accumulated, ...res.items]
        // BE MaximumPageSize = 100, so nếu items.length < 100 thì đó là page cuối
        if (res.items.length < 100) {
          return of(allItems)
        }
        return this.loadAllUsers(pageNumber + 1, allItems)
      }),
  ...
  updatePolicy(payload: PolicyResponse): Observable<void> {
    return this.http.put<void>(`${this.base}/Policies`, payload).pipe(catchError(() => of(void 0)))
  }
