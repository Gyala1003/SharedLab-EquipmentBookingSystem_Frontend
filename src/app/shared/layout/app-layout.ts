...
@Component({
  selector: 'app-layout',
  imports: [DatePipe, NgClass, RouterLink, RouterLinkActive, RouterOutlet, IconComponent, ModalComponent, TranslatePipe],
  template: `
    <div class="min-h-screen bg-[#f5f7fb] text-slate-900">
      ...
      <aside class="fixed inset-y-0 left-0 z-40 flex w-[292px] flex-col border-r border-white/10 bg-[#101936] text-white shadow-2xl shadow-slate-900/20 transition-transform duration-300 lg:translate-x-0" [ngClass]="mobileOpen() ? 'translate-x-0' : '-translate-x-full'">
        <div class="flex h-20 shrink-0 items-center gap-3 border-b border-white/10 px-5">
          <div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-400 to-cyan-300 text-[#111a3a] shadow-lg shadow-sky-500/20"><app-icon name="flask" [size]="24" /></div>
            ...
            <div class="mb-5">
              ...
              <nav class="mt-2 space-y-1">
                @for (item of group.items; track item.route) {
                  <a [routerLink]="item.route" routerLinkActive="bg-gradient-to-r from-sky-500 to-sky-600 text-white shadow-lg shadow-sky-950/25" class="group flex items-center gap-3 rounded-2xl px-3.5 py-2.5 text-[13px] font-bold text-white/64 transition hover:bg-white/10 hover:text-white" (click)="mobileOpen.set(false)">
        ...
        <div class="shrink-0 border-t border-white/10 p-3">
          @if (store.user(); as user) {
            <div class="rounded-[22px] border border-white/10 bg-white/[0.06] p-3.5 backdrop-blur">
              <a routerLink="/app/profile" class="flex items-center gap-3 rounded-xl transition hover:bg-white/[.04]" (click)="mobileOpen.set(false)"><div class="flex h-10 w-10 shrink-0 items-center justify-center rounded-2xl bg-gradient-to-br from-sky-400 to-cyan-300 text-xs font-black text-[#111a3a]">{{ initials(user.fullName) }}</div><div class="min-w-0 flex-1"><p class="truncate text-sm font-black">{{ user.fullName }}</p><p class="mt-0.5 truncate text-[10px] font-semibold text-white/40">{{ roleLabel(user.roleName) }}</p></div><app-icon name="chevron-right" [size]="15" /></a>
              <div class="mt-3 flex items-center justify-between border-t border-white/10 pt-3"><span class="inline-flex items-center gap-2 text-[10px] font-bold text-emerald-300"><span class="h-2 w-2 rounded-full bg-emerald-400 shadow-[0_0_0_4px_rgba(52,211,153,.12)]"></span> {{ 'sidebar.connected' | t }}</span><button type="button" class="rounded-xl p-2 text-white/45 hover:bg-white/10 hover:text-white" title="{{ 'sidebar.logout' | t }}" (click)="logout()"><app-icon name="logout" [size]="17" /></button></div>
            </div>
          }
        </div>
      </aside>
      ...
      <div class="min-h-screen lg:pl-[292px]">
        <header class="sticky top-0 z-20 flex h-20 items-center gap-4 border-b border-slate-200/80 bg-white/88 px-4 backdrop-blur-xl sm:px-6 lg:px-8">
          ...
          <div class="min-w-0 flex-1"><p class="text-[10px] font-black uppercase tracking-[0.2em] text-sky-600">{{ 'header.workspace' | t }}</p><p class="mt-1 truncate text-sm font-bold text-slate-600">{{ 'header.subtitle' | t }}</p></div>
          <div class="hidden items-center gap-1 rounded-full border border-slate-200 bg-white p-1 sm:flex">
            <button type="button" class="rounded-full px-2.5 py-1 text-xs font-black" [ngClass]="languageStore.lang() === 'vi' ? 'bg-sky-50 text-sky-600' : 'text-slate-400'" (click)="languageStore.setLang('vi')">🇻🇳 VN</button>
            <button type="button" class="rounded-full px-2.5 py-1 text-xs font-black" [ngClass]="languageStore.lang() === 'en' ? 'bg-sky-50 text-sky-600' : 'text-slate-400'" (click)="languageStore.setLang('en')">🇬🇧 EN</button>
          </div>
          <a routerLink="/app/calendar" class="hidden h-11 items-center gap-2 rounded-2xl border border-slate-200 bg-white px-4 text-xs font-black text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200 hover:text-sky-600 sm:flex"><app-icon name="calendar" [size]="18" /> {{ 'header.viewCalendar' | t }}</a>
          <a routerLink="/app/notifications" class="relative flex h-11 w-11 items-center justify-center rounded-2xl border border-slate-200 bg-white text-slate-600 shadow-sm transition hover:-translate-y-0.5 hover:border-sky-200 hover:text-sky-600 hover:shadow-md" aria-label="{{ 'header.notifications' | t }}"><app-icon name="bell" [size]="20" />@if (badge.count() > 0) { <span class="absolute right-1.5 top-1.5 h-2.5 w-2.5 rounded-full border-2 border-white bg-rose-500"></span> }</a>
          <a routerLink="/app/profile" class="hidden items-center gap-3 rounded-2xl px-2 py-1.5 transition hover:bg-slate-50 md:flex">@if (store.user(); as user) { <div class="text-right"><p class="text-sm font-black text-slate-800">{{ user.fullName }}</p><p class="mt-0.5 text-[10px] font-bold text-slate-400">{{ roleLabel(user.roleName) }}</p></div><div class="flex h-10 w-10 items-center justify-center rounded-2xl bg-sky-50 text-sm font-black text-sky-700">{{ initials(user.fullName) }}</div> }</a>
        </header>

        <main class="mx-auto w-full max-w-[1580px] p-4 sm:p-6 lg:p-8"><router-outlet /></main>
      </div>

      <app-modal [open]="pendingCheckoutOpen()" title="Bạn có phiên sử dụng phòng lab chưa checkout" subtitle="Hệ thống phát hiện phiên chưa hoàn tất sau giờ kết thúc booking." (close)="pendingCheckoutOpen.set(false)">@if (pendingBooking; as booking) { <div class="grid gap-3 rounded-2xl bg-slate-50 p-4 text-sm"><div class="flex justify-between"><span class="text-slate-400">Booking</span><span class="font-black text-slate-800">#BK-{{ booking.bookingId.toString().padStart(5,'0') }}</span></div><div class="flex justify-between"><span class="text-slate-400">Giờ kết thúc</span><span class="font-bold text-slate-700">{{ booking.endTime | date:'HH:mm dd/MM/yyyy' }}</span></div></div><div class="mt-5 flex flex-wrap justify-end gap-2"><button class="btn-secondary" (click)="snoozePendingCheckout()">Tiếp tục sử dụng</button><button class="btn-primary" (click)="confirmPendingCheckout()"><app-icon name="logout" [size]="16" /> Đăng ký hoàn tất (Checkout)</button></div> }</app-modal>
    </div>
  `,
})
export class AppLayoutComponent implements OnInit {
  ...
  private readonly groups: readonly NavGroup[] = [
    {
      label: 'Tổng quan',
      items: [
        { labelKey: 'nav.home', icon: 'home', route: '/app/home', roles: ['Requester'] },
        { labelKey: 'nav.dashboard', icon: 'dashboard', route: '/app/dashboard', roles: ['Admin', 'LabManager'] },
        { labelKey: 'nav.resourceCalendar', icon: 'calendar', route: '/app/calendar' },
      ],
    },
    {
      label: 'Tài nguyên',
      items: [
        { labelKey: 'nav.labs', icon: 'building', route: '/app/labs' },
        { labelKey: 'nav.equipment', icon: 'microscope', route: '/app/equipments' },
        { labelKey: 'nav.maintenanceSchedule', icon: 'wrench', route: '/app/management/maintenances' },
      ],
    },
    {
      label: 'Cá nhân',
      items: [
        ...
        { labelKey: 'nav.myViolations', icon: 'alert', route: '/app/violations/my' },
        { labelKey: 'nav.notifications', icon: 'bell', route: '/app/notifications', badge: 'notifications' },
        { labelKey: 'nav.profile', icon: 'user', route: '/app/profile' },
        { labelKey: 'nav.policy', icon: 'shield', route: '/app/policy' },
      ],
    },
    /*
    {
    ...
    */
    {
      ...
      roles: ['Admin'],
      items: [
        { labelKey: 'nav.users', icon: 'users', route: '/app/admin/users' },
        { labelKey: 'nav.departments', icon: 'building', route: '/app/admin/departments' },
        // { label: 'Quy tắc ưu tiên', icon: 'layers', route: '/app/admin/priority-rules' },
        { labelKey: 'nav.sendNotification', icon: 'send', route: '/app/admin/notifications/send' },
  ...
  ngOnInit(): void {
    ...
    // Đây là cơ chế phát hiện tạm thời phía Frontend, chỉ chạy khi người dùng thực sự mở lại app; không thay thế được background job phía Backend.
  ...
  private detectPendingCheckout(userId: number): void {
    ...
    this.api.bookingsByUser(userId).subscribe((bookings) => {
      const candidates = bookings.filter((b) => b.status === 'Approved' && +new Date(b.endTime) < now).sort((a, b) => +new Date(b.endTime) - +new Date(a.endTime)).slice(0, 3)
  ...
  protected confirmPendingCheckout(): void {
    ...
    this.api.checkOut(log.logId, actualCheckoutIso).subscribe({
      next: () => { this.toast.success('Check-out thành công'); this.checkLateAndReportViolation(booking, actualCheckoutIso) },
  ...
  private checkLateAndReportViolation(booking: BookingResponse, actualCheckoutIso: string): void {
    ...
    this.api.createViolation({ userId: booking.userId, bookingId: booking.bookingId, violationType: 2 }).subscribe({
      next: () => this.toast.info('Đã ghi nhận vi phạm trả phòng muộn (tạm thời tính ở Frontend, sẽ điều chỉnh khi Backend hỗ trợ)'),
      error: () => {},
    })
  }
}
